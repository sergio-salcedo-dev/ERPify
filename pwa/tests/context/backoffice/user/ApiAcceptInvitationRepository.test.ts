...
describe("ApiAcceptInvitationRepository.accept", () => {
  it("maps a 204 to accepted and posts {token, password, _token} to the accept endpoint", async () => {
    ...
    expect(post).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ token: COMMAND.token, password: COMMAND.password }),
    );
    // The stateless CSRF nonce is generated by the adapter and satisfies the
    // >= 24 char contract (a v7 UUID is 36 chars).
    const body = post.mock.calls[0][1] as { _token: string };
    expect(typeof body._token).toBe("string");
    expect(body._token.length).toBeGreaterThanOrEqual(24);
  });
