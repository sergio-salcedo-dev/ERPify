...
/** How many selected emails the confirm dialog spells out before "+N more". */
...
/**
 * Floating selection bar pinned to the bottom of the content column while users
...
export function UsersBulkBar({
  ...
  return (
    <section
      className="users-list__bulk-bar border-border bg-card shadow-elevation-4 sticky bottom-6 z-30 mx-auto flex w-max max-w-[calc(100vw-2rem)] flex-wrap items-center gap-3 rounded-xl border px-4 py-2 sm:flex-nowrap"
      aria-label="Bulk actions"
      data-testid="users-list__bulk-bar"
    >
      <span className="text-foreground text-sm font-medium" data-testid="users-list__bulk-count">
        {count} selected
      </span>
      <div className="ml-auto flex items-center gap-2">
        <Button
          ...
          data-testid="users-list__bulk-clear"
        >
          Clear
        </Button>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          />
          <DialogContent data-testid="users-list__bulk-delete-dialog">
            <DialogHeader>
              <DialogTitle>
                Delete {count} {noun}
              </DialogTitle>
            ...
            {shownNames.length > 0 ? (
              <ul
                className="users-list__bulk-delete-names text-muted-foreground list-none space-y-1 p-0 text-sm"
                data-testid="users-list__bulk-delete-names"
              >
                {shownNames.map((name) => (
            ...
            <DialogFooter>
              ...
              <Button
                ...
                data-testid="users-list__bulk-delete-confirm"
              >
                Delete
              </Button>
            </DialogFooter>
