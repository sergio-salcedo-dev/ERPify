import type { UsersView } from "./UsersViewToggle";
...
const SKELETON_TESTID = "users-list__skeleton";
 ...
 */
export function UsersListSkeleton({ view, rows = 6 }: Readonly<UsersListSkeletonProps>) {
  const keys = SKELETON_ROW_KEYS.slice(0, Math.min(rows, SKELETON_ROW_KEYS.length));

  if (view === "cards") {
    return (
      <ul
        className="users-list__skeleton grid list-none grid-cols-1 gap-4 p-0 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4"
  ...
  return (
    <div
      className="users-list__skeleton border-border overflow-hidden rounded-md border"
      data-testid={SKELETON_TESTID}
      aria-hidden="true"
    >
      {keys.map((key) => (
        ...
        >
          ...
          <div className="bg-muted hidden h-4 w-24 rounded sm:block" />
          <div className="bg-muted h-4 w-20 rounded" />
          <div className="bg-muted hidden h-4 w-32 rounded md:block" />
          <div className="bg-muted ml-auto h-7 w-16 rounded" />
        </div>
      ))}
    </div>
  );
