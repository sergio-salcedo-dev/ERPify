...
- **Redirect targets from untrusted sources** (a `?next=` param, location state, an API payload) — `safeHref` blocks dangerous schemes but still allows an absolute `https://evil.com`, so a redirect target MUST additionally pass `safeInternalPath(value, fallback)` from `@/lib/safeInternalPath`, which accepts only a root-relative in-app path (rejects absolute, protocol-relative `//host`, and backslash-smuggled `/\host`). The login return-URL flow is the reference (`RequireAuth` writes `?next=`, `LoginForm` reads + validates it).
