...
| `api/src/Shared/Infrastructure/Persistence/Doctrine/Search/` | `FilterApplier` + `SearchFieldMap` — shared search-filter plumbing (mandatory allow-list); `DoctrineSearchEngine` (keyset query-shaper, 8-step pipeline) + `RowUniquenessGuard`, composing the pure `Keyset/` kernel. As of PR3 it governs the live HTTP read-path (Bank), producing a link-agnostic `Page` with opaque cursors; `Shared/Infrastructure/Http/Responder/SearchResponder` is the single compositor of the cursor-only envelope. The legacy `Paginator` stack is decoupled (reachable only behind the `pagination_mode=legacy` valve) until PR4 |
