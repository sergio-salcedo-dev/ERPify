...
make sf.routes.manifest             # Regenerate api/.route-manifest.json from the PROD router.
make sf.config.reference            # Regenerate api/config/reference.php from this vendor tree.
...
make db.test.shell                  # Interactive psql on the test database (db.shell opens the runtime one).
...
make bmad.skills.sync.dry-run       # Report drift between the primary's .claude/skills/bmad-* and its .agent/skills.
...
`git clone` never gave you a working BMad install and tracking one root only made it look as though it had.
The roots are regenerated by the installer from `_bmad/`; `make worktree.create` copies the primary's
`.agent/skills/bmad-*` into a new worktree, the same place it links `_bmad` from, plus the primary's
`.claude/skills/bmad-loop-*` — the only root `bmad-loop init` writes, so without them a sweep started from a
worktree answers `Unknown command: /bmad-loop-sweep`.

**The sync exists because an update reaches only the checkout it ran in.** Run the installer in a worktree
and the primary keeps serving whatever it had — measured on 2026-09-20, the day after the 6.12.0 update
(#945, `3d3ef122`, 2026-09-19), the primary was serving a tree roughly two months old: 73 skill directories
against 77, 790+ lines of `diff -rq`. **The command acts on the primary whichever checkout invokes it**,
since `CLAUDE.md` requires the work to happen in a worktree and a worktree is correct by construction;
`c='--root <path>'` overrides. It **replaces** rather than merging, refuses a non-directory `bmad-*` entry
before touching anything, and restores from its own backup — verifying the restore — if the copy fails.

Its guardrail is **conservative, because nothing can arbitrate provenance**. A `bmad-*` directory the source
lacks is either a skill the installer retired or one somebody wrote by hand, and every extra stops the run
whatever it looks like; `--force` removes it after the backup holds it. An earlier version asked git which,
while `.agent/skills` was tracked — and asking the PATH's history separates paths, not authors, so
hand-written content at a retired skill's name was measured deleted with exit 0. Noisier after an update
that retires a skill, and strictly safer. Two bounds: the universe is `bmad-*/`, so a hand-written skill
under any other name is protected by the glob rather than by this check, and the comparison prunes
`__pycache__`/`*.pyc`, because a skill that has merely been *run* would otherwise report as drift for ever.
