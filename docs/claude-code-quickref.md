...
- **A dev stack never comes back on its own, and every service is capped.** `compose.yaml` declares `restart: unless-stopped` for the deployed shape; `compose.dev.yaml` overrides it to `"no"` and adds a `deploy.resources.limits.memory` per service — `PHP_MEM_LIMIT` 4g, `PWA_MEM_LIMIT` 4g, `DB_MEM_LIMIT` 1g, `WORKER_MEM_LIMIT` 512m, `MAILPIT_MEM_LIMIT` 256m, each overridable per run (`make docker.up PHP_MEM_LIMIT=8g`). Both are dev-only: staging and prod load `compose.prod.yaml`, which keeps `unless-stopped` and its own tighter limits. One stack per worktree times `unless-stopped` is what makes the restart policy matter: every stack ever started resurrects on daemon start, and a 30 GB host with seven worktrees was back at 32 containers and 20 GB in use three minutes after a reboot, then froze on swap exhaustion (`Free swap = 52kB`, `all_unreclaimable? yes`) — the OOM killer answering with a Chrome helper worth 271 MB, which relieves nothing and leaves the freeze to repeat. **Size a cap from the cgroup's `memory.peak`, never from sampling `docker stats`**: sampling every ~2s read php at 827 MiB, and a 2g cap built on that number was OOM-killed by Rector's parallel run within the minute (`CONSTRAINT_MEMCG`, exit 137). `memory.peak` reads 6157 MiB for the same container under a 12g ceiling, nearly all reclaimable page cache: at 4g `php.quality`, `php.test` and `php.unit.coverage` all pass and the peak sits at the cap. A container that reaches its cap is OOM-killed alone and **stays down** — `make docker.up` brings it back.
