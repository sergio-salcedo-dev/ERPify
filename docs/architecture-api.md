...
- **A person's aggregate id may not ride a persisted transport.** `async` and `failed` are Doctrine tables
  ...
  and cross-checked against `routing` by `make php.lint.persistent-transport`, which resolves each routing
  key to the events Messenger would really send through it (parents, interfaces, namespace wildcards, `'*'`
  and `#[AsMessage]`), not to a class name. It says nothing about `event_store`, which keeps the real
