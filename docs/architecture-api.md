...
└── Shared/
    ├── Application/    { Http/Search, Mailer, Problem, UseCase, Validation }
    ├── Domain/         { Aggregate, Entity, Enum, Exception, Search, Uuid }
    ├── Event/          { Domain, Application, Infrastructure }   # event backbone: DomainEvent, EventBus, event store, projections
    ├── Guzzle/         { Enum }
    ├── Infrastructure/ { Http, Mailer, Persistence, Serializer, Uuid }
