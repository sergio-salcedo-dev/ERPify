...
- **[adr/event-driven-architecture.md](./adr/event-driven-architecture.md)** — ADR: `EventBus` port + transactional outbox closing the domain-event dual-write; the three axes (state/atomic, actor/best-effort, read), the no-broker-direct invariant, `wrapInTransaction` as a transitional detail pending the `CommandBus` middleware (#263), and the `php.lint.event-bus` enforcement gate
- **[adr/behat-event-observability-test-contexts.md](./adr/behat-event-observability-test-contexts.md)** — ADR: Behat step tooling for the event/outbox and log-line seams, ported from internal bundles by capability not class; in-memory `MessengerContext` (D2) vs opt-in doctrine-transport `OutboxContext` for atomicity (D3), `SpyLogger` for RFC 9457 log assertions (D4), additive `ConsumeContext` (D5), rate-limiter generalization deferred by Rule of Three (D6)
