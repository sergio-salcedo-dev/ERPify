...
- **[adr/aggregate-receives-the-instant.md](./adr/aggregate-receives-the-instant.md)** — ADR: the application layer reads the injected `Clock` once per operation and hands the instant inward; `AggregateRoot`, every stamping factory/mutator and `DomainEvent::$occurredOn` take it as an argument; ambient `SystemClock` + its initializer and the `Timestamped` setters deleted; `php.lint.wall-clock` refuses a second time source in `api/src` (injecting `Clock` into the aggregate and a Doctrine timestamp listener rejected)
