...
- **[adr/image-read-failure-signal-bound.md](./adr/image-read-failure-signal-bound.md)** — ADR (accepted): the read path's storage-failure signal is deliberately left **unbounded**, superseding an epic acceptance criterion that ordered a bound. Three measurements moved it: the premise was false (under the mandated `findById()` → `read()` order a random identifier never reaches storage, so the producers need an orphaned row or a deployment fault), the `debug` escape the criterion offered is discarded by a gate pinning the handler at `info`, and every available bound sits at the wrong control point — a 5xx also writes an `error` record on the default channel that activates the buffered handler and is unreachable from this module. Records that nothing listens to `observability` (no collector, Sentry handler commented out), so the signal is forensic and not an alarm; that the real control point is sink isolation, which is transverse infrastructure and not this module's; and that frequency cannot be measured because there is no production deployment — only cost per event
- **[adr/image-read-route-contract.md](./adr/image-read-route-contract.md)** — ADR (accepted): the contract of `GET /api/v1/images/{imageId}` while no consumer exists — authentication (`IS_AUTHENTICATED_FULLY`) as a **provisional** frontier with no ownership voter (a permission granted to no role was measured and refused: 403 for everyone deletes the slice), `ImageId` never an authorization mechanism, and the first real consumer owing its own policy, forced into its diff by `ImageConsumerAuthorizationGateTest`; no `audit_log` row, because person/non-person is classified per consumer (`BankLogo` ≠ `UserAvatar`) and never per `Image`; `Cache-Control: private, max-age=3600, immutable` — one hour rather than a year, since an immutable identifier does not make bytes whose contract is reliable deletion indefinitely cacheable (a prior, not a measurement; a zero deletion SLA means `no-store`); `Range` ignored; canonicalization as an implicit v1, versioned on its first change in merged code
