...
`InvalidSearchCriteria` covers *well-formed but semantically invalid* search input — bad filters (unknown/un-filterable field, operator not allowed for the field, value of the wrong format or blank), an un-sortable order field, or out-of-range pagination. Concrete exceptions under `api/src/Shared/Domain/Search/Exception/`: `UnknownSearchField` → `unknown-search-field`, `UnsupportedSearchOperator` → `unsupported-search-operator`, `InvalidSearchValue` → `invalid-search-value`, `UnknownSortField` → `unknown-sort-field`, `InvalidPagination` → `invalid-pagination`. The shared filter applier throws the first two plus the format checks of `invalid-search-value`; the shared search repository throws `unknown-sort-field` when `sort` falls outside its `sortFieldMap()` allow-list (before any SQL, so the field never reaches DQL; `context` = `{field}` only). `invalid-search-value` fires for any value a field mapping rejects — a malformed UUID against a `requiresUuidValues` column or a malformed/lax datetime against a `requiresDateTimeValues` column (each would otherwise hit Postgres as a 22xxx → 500) — and is also raised by the domain `Filter` constructor on a blank value (empty after a Unicode-aware trim), so the invariant holds across every adapter (HTTP, CLI, handlers); `context` = `{field, position}`, never the value. `invalid-pagination` comes from the `SearchCriteria` constructor when `page`/`limit` leaves `[1, max]` (`MAX_PAGE`/`MAX_LIMIT`) — likewise all-adapter — while the HTTP DTO `SearchQuery` rejects the same earlier as 422 `validation-failed`; `context` = `{page, max}`/`{limit, max}`, bare integers only. The whole family maps to **422**, joining wire-DTO `validation-failed` under the pragmatic convention (Rails, Laravel, GitHub) that 422 covers any well-formed-but-unprocessable input. **400** is reserved for a malformed request *target* — a path id that is not a well-formed UUID (`InvalidInput` → `invalid-uuid`); the search marker travels only on `DomainException` and never collides with it.
