...
The mapping is the constant `ProblemDetailsFactory::MARKER_STATUS_MAP` (see [`api/src/Shared/ErrorContract/Application/ProblemDetailsFactory.php`](../api/src/Shared/ErrorContract/Application/ProblemDetailsFactory.php) lines 112–121). The default `type` per marker is `MARKER_DEFAULT_TYPE_MAP` (lines 123–132). **Do not duplicate the values here — this table is a navigation aid; the source is the constant** (NFR25).
...
|-----------|--------|-----------|---------------------------------------|
| `DekDestroyed`             | `dek-destroyed`               | `SodiumEnvelopeEncryptor::decrypt` (E3 read path) / `mint` tombstone race              | No             |
| `DecryptionFailed`         | `decryption-failed`           | `SodiumEnvelopeEncryptor::decrypt`/`unwrap` — AEAD open failure / KEK-version mismatch | No             |
...
| `InvalidKek`               | `invalid-kek`                 | `SodiumEnvelopeEncryptor` constructor — `AUDIT_KEK` length check                       | No (boot-time) |
