...
| `DEACTIVATED`               | post-auth | **403 `forbidden`** (generic)  | Same mechanism, but `AccountDeactivated` leaves `type()` empty so the `Forbidden` default applies — a retired identity has no actionable next step, and the specificity boundary is identity proof, not the error kind.                                                                                            |

Both 403 walls carry no `AccessDeniedException` in their chain, so `UnauthenticatedAccessListener` (which would rewrite that to a 401) leaves them alone; both abort authentication, so **no session cookie or resumable token is minted** — the "no session before admission" guarantee is structural, not cleanup. `Forbidden extends ClientError`, so both walls are Sentry-suppressed and log at `warning`. No new marker interface is introduced (both reuse `Forbidden`), so the drift gate does not fire — this section is the manual record NFR26 requires. The `invalid-token` type and the operational 503-family gate are out of scope here.
