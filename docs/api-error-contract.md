...
A second failure exists and is not a token case: the accept flow re-authenticates the device post-commit through [`ReauthenticateDevice`](../api/src/Iam/Identity/Infrastructure/Security/ReauthenticateDevice.php), uncaught, so an identity walled between the commit and the mint surfaces **403 `account-suspended`** / **`account-deactivated`** on this route — the same two types the login and reset surfaces raise, reusing `Forbidden`, so no marker is added. It is listed apart from the table above because it is not one of the five opacity cases and answers *after* the invitation was accepted.
...
| Identity walled **after** the commit, during the re-login | **the same two 403s — but everything already happened** | `CompletePasswordReset` re-authenticates the device post-commit through [`ReauthenticateDevice`](../api/src/Iam/Identity/Infrastructure/Security/ReauthenticateDevice.php), uncaught. A status change landing inside that window (the revoke plus a blocking SMTP send wide) therefore produces a response **byte-identical to the pre-commit wall above**, after the token was consumed, the credential replaced, every session revoked and the notice mailed. A client cannot tell the two apart, and the rows above would tell it to retry with the old password, which no longer works. Narrow — it needs an administrator acting inside that window — and named here because nothing in the wire response distinguishes it. The change surface does not share this: it contains the same failure at `critical` and still answers 204, because there the credential is already gone and a non-2xx would invite an impossible retry. |
...
The re-login that follows a successful change is [`ReauthenticateDevice`](../api/src/Iam/Identity/Infrastructure/Security/ReauthenticateDevice.php), shared with the reset and invitation-accept flows. It re-reads the aggregate and applies the same `ensureActive()` wall before minting, because `Security::login()` runs only the `checkPreAuth` half of the admission wall while the session-minting listener fires regardless — so without it the window between the revoke and the login (a blocking, unrouted SMTP send wide) could mint an `ACTIVE` session for an identity just suspended. On the change surface that refusal is contained: the credential is already committed, so the response stays **204** and the failure goes to the log at `critical`. On the reset and accept surfaces, which have no such containment, it surfaces as the 403 above.
