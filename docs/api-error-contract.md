...
| `SentryBundle\…\ErrorListener`      | `kernel.exception` | 128      | all (prod-only)    |

The Sentry `ErrorListener` (prod-only) runs first at `128` but only *captures* the throwable — it sets no response, so `ExceptionResponder` (16) still builds the RFC 9457 body unchanged.
