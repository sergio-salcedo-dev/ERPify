...
The same adapter translates a second DBAL failure, to the opposite half of the contract: a foreign key rejected at flush becomes [`Shared/Persistence/Domain/Exception/ReferentialIntegrityViolation`](../api/src/Shared/Persistence/Domain/Exception/ReferentialIntegrityViolation.php) — the existing `Conflict` marker, `type` **`referential-integrity-violation`** → **409**. No new marker interface is introduced, so the drift gate does not fire; this paragraph is the manual record NFR26 requires. **409 and not 503**: unlike a deadlock, the identical request repeated cannot succeed until the referencing rows are gone, so there is something for the caller to reconcile and "try again" would be false advice. It is deliberately generic — a use case that can name the dependents owes a better answer and catches it to give one (bank deletion answers 409 `bank-in-use` with its own account count); this exists so every *other* path arrives as a 409 a client can reason about instead of the bare 500 an untranslated DBAL exception produces. **Scope:** as above, the sanctioned seam only.
...
**Sentinel (`REDACTED`), not strip** — the opposite of the map denylist, deliberately: a URI's diagnostic value is its shape, so `filters[0][field]=actorId` survives while its value does not, and an operator can still tell a filtered request from an unfiltered one. It is the same token `api/frankenphp/Caddyfile` writes into the access log, so both read alike.

The sink is what makes this a leak rather than verbosity: in prod Monolog writes to `php://stderr` behind `fingers_crossed`, so one 5xx flushes the buffered WARNING lines of unrelated 4xx into the default json-file Docker driver — no rotation, no TTL, no owner of erasure. A person id landing there outlives the erasure the application confirmed to the subject.
