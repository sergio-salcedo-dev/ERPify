...
Both 403 walls carry no `AccessDeniedException` in their chain, so `UnauthenticatedAccessListener` (which would rewrite that to a 401) leaves them alone; both abort authentication, so **no session cookie or resumable token is minted** — the "no session before admission" guarantee is structural, not cleanup. `Forbidden extends ClientError`, so both walls are Sentry-suppressed and log at `warning`. No new marker interface is introduced (both reuse `Forbidden`), so the drift gate does not fire — this section is the manual record NFR26 requires. The `invalid-token` type is out of scope here; the operational 503-family gate and the `session-expired` 401 are documented under the marker table above.
