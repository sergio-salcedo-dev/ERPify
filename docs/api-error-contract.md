...
- **`correlation-id`** — UUIDv7 minted per **REQUEST**. Source: `CorrelationIdListener::__invoke` (`kernel.request`, priority `1024`). **Always minted by the server: an inbound `X-Correlation-Id` is ignored, not validated.** The same value becomes the `correlation_id` column of every `audit_log` row the request writes, and the back office renders it per row and pivots the forensic trail on it ("Follow this correlation") — so a caller able to choose it could collapse unrelated rows under one identifier, or file their own rows under another request's, and no check applied to a single request can tell a reused id from a fresh one. Mirrored in the body's `correlation-id` field and written to the `X-Correlation-Id` response header. In the logs it reaches only the emitters that put it there — `ExceptionResponder`'s error line and `SearchObservabilityListener`'s `observability` lines — because **no Monolog processor injects it**; the full trail of a request is in `audit_log`, grouped by this same column. A caller needing distributed tracing needs an identifier of its own rather than authority over this one.
