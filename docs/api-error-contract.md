...
400 (not 401 or 404): a dead token is a malformed request *target*, like a malformed route UUID (`invalid-uuid`) — 401 would conflate with a credentials failure, and 404 would leak whether the invitation exists. The `InvalidInput` marker keeps the status uniform across the five cases, which is the whole opacity requirement ([`identity-invitation-lifecycle.md`](adr/identity-invitation-lifecycle.md) D11). The invited email is never surfaced, and the raw token never appears in a response, log or event. On success the flow answers **204** with the session cookie (mirroring login) — never a body that could echo identity.
...
- **`correlation-id`** — UUIDv7 minted per **REQUEST**. Source: `CorrelationIdListener::__invoke` (`kernel.request`, priority `1024`). **Always minted by the server: an inbound `X-Correlation-Id` is ignored, not validated.** The same value becomes the `correlation_id` column of every `audit_log` row the request writes, and the back office renders it per row and pivots the forensic trail on it ("Follow this correlation") — so a caller able to choose it could collapse unrelated rows under one identifier, or file their own rows under another request's, and no check applied to a single request can tell a reused id from a fresh one. Mirrored in the body's `correlation-id` field and written to the `X-Correlation-Id` response header. In the logs it reaches only the emitters that put it there — `ExceptionResponder`'s error line and `SearchObservabilityListener`'s `observability` lines — because **no Monolog processor injects it**; the full trail of a request is in `audit_log`, keyed by this same column. A caller needing distributed tracing needs an identifier of its own rather than authority over this one.
