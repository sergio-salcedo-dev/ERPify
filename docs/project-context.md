...
| Static analysis           | PHPStan 2 (`level: max`, sole type gate), Rector 2; Psalm 6.16 taint-only (security dataflow) | Configs: `api/tools/phpstan/phpstan.neon`, `api/rector.php`, `api/tools/psalm/psalm-taint.xml`. Psalm's general analysis was retired — it now runs taint-only in the `api-taint` CI job.                                                                                                                                                                                 |
...
- **PHP**: PHP-CS-Fixer (`api/.php-cs-fixer.php`), PHPCS, PHPStan 2 (`api/tools/phpstan/phpstan.neon`, `level: max` — sole type gate), Rector 2, PHPMD. Run all via `make php.quality`. Don't hand-format against these tools. Psalm 6.16 runs **taint-only** (`make php.psalm.taint`, `api-taint` CI job), not in `php.quality`.
