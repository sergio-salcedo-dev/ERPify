...
| E2E tests                 | **Behat 4** in the app Composer tree                                                          | Declared in `api/composer.json` under `require-dev`; configured by `api/behat.dist.php`. Runs from `api/vendor/bin/behat`, like PHPUnit. The constraint is `^4.0@alpha` and the installed release is a **prerelease** — a Behat minor can change the `Behat\Config\*` API `behat.dist.php` is written against, so treat a bump as a change to the suite.                                                                                                                                                                                                                                 |
...
| Concern              | Technology (version)                                                             | Key Constraint for Code Generation                                                                                                                                                                                |
|----------------------|----------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Runtime (container)  | **Node 26** (Debian trixie, digest-pinned)                                       | `pwa/Dockerfile` pins `node@sha256:…` (currently `node:26.8.2-trixie`) — not a tag, and not Alpine. `engines.node` is `^24.15.0 \|\| >=26.0.0`; keep it in step with the pin.                                       |
...
| Styling              | **Tailwind 4.3** (via `@tailwindcss/postcss`) + Shadcn 4.19                      | **No `tailwind.config.js`** — Tailwind 4 is CSS-first. Configuration lives in `pwa/src/app/globals.css` via `@theme {}` / `@config`. Do **not** generate v3-style JS config.                                      |
...
| Unit tests           | **Vitest 5**                                                                     | Config: `pwa/vitest.config.ts` (v5 config API differs from v1/v2). Command: `make pwa.test.unit c='src/context/foo/bar.test.ts'`.                                                                                 |
| E2E tests            | **Playwright 1.63**                                                              | Config: `pwa/playwright.config.ts`. Runs on the **host**, never in a container. `baseURL` = `PLAYWRIGHT_BASE_URL` ?? (`CI` ? `https://localhost` : `http://127.0.0.1:3000`). The Compose stack is a prerequisite either way (the target seeds through it and the API fixtures default to `https://localhost`); what changes is the **front-end**: by default Playwright serves it from a host-spawned `dev:e2e` on `:3000`, not the containerised Next behind FrankenPHP. |
| Testing libs         | @testing-library/react 16, jest-dom 7, jsdom 30                                   | —                                                                                                                                                                                                                 |
| Lint / format        | ESLint 10.8 + `@next/eslint-plugin-next` 16.3 + `eslint-plugin-jsx-a11y` 6.10 + `eslint-config-prettier`, Prettier 3.9 | Run via `make pwa.quality`. The jsx-a11y set follows upstream's `recommended`; its identity is pinned by `pwa/tests/eslint/jsxA11yRuleSetGate.test.ts`, so a bump that moves a rule in or out reds that test naming it. `settings["jsx-a11y"].components` maps the design-system wrappers, without which the set only sees intrinsic elements. `reportUnusedDisableDirectives` is an error, so a stale `eslint-disable` reds `pwa.lint.dry-run` and CI — **not** `make pwa.quality`, which runs `--fix` and deletes it silently. |
...
- Drive the app through Symfony's `KernelBrowser` (`api/tests/Behat/Context/HttpRequestContext.php`). There is **no Mink** in this tree — `behat/mink*` appears only in `friends-of-behat/symfony-extension`'s `suggest` block, never as an installed package.
...
- CI runs the same suites as `make ci.test`, but split across jobs and invoked one by one (`php.unit.coverage`, `php.behat`, `pwa.test.unit.coverage`, `pwa.test.e2e`); the target name itself appears nowhere in `.github/workflows/ci.yml`. Verify locally with `make app.test` before pushing.
