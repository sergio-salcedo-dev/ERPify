...
**An injected `FixedClock` must agree with the ambient one, and the double enforces it.** Aggregates stamp `createdAt`/`updatedAt` off the ambient `SystemClock`; a use case computes an expiry off the `Clock` it was handed. A test injecting `FixedClock` at one instant while the ambient clock stands at another builds rows production cannot produce — a session created in 2050 that expires in 2026 — and stays green, because nothing compares the two. So `FixedClock::now()` throws a `LogicException` naming both instants when it is not itself the ambient clock and its instant differs from `SystemClock::now()` (compared as instants, so one moment written in two zones passes). Align with `SystemClock::set($clock)` before the subject runs, or seed the double from `SystemClock::now()` when the test needs no literal instant. A test that advances time installs the later clock with `SystemClock::set()` **before** invoking the subject that reads it; aggregates built earlier keep their earlier stamp. To give a row a chosen `createdAt`, build it under a clock frozen at that instant — `Timestamped` has no setters, and a row whose `updatedAt` differs from its `createdAt` gets it from a real mutation run under the later clock. The check runs on read, not on construction, because only the read tells a divergence apart from a test preparing two instants — so the two clocks disagree in a red whenever the subject reads the injected clock, and an injected clock the subject never reads stays green however wrong it is. It compares against the ambient `SystemClock` only; Symfony's global clock, which `pin()` also sets and the container's `clock` service reads, is never consulted. There is no opt-out. It sees only this double: a bare `new DateTimeImmutable()`, `DomainEvent`'s `occurredOn` default, a Symfony `MockClock` handed to a service directly, and the Behat lane all read past it. `FixedClockTest` pins where it fires and where it must not.

**Blind spots.** The pin owns those two sources and no others, and `FreezeSystemClockExtensionTest` witnesses both — but not the *leading* edge, which the trailing pin satisfies on its own; the case it defends (a test that skipped or threw in `setUp()` and so never reached `Finished`) cannot be staged in a suite that forbids ordering dependencies, and rests on argument alone. A bare `new DateTimeImmutable()` or `time()` reads past both — including `Shared\Event\Domain\DomainEvent`'s `$occurredOn ?? new DateTimeImmutable()` default. Beyond the `FixedClock` check above, nothing gates the rule: the four above were found by moving the pinned instant and reading what broke, which is the check to repeat rather than a green to trust. Postgres keeps a clock nothing here touches, and Behat boots from its own bootstrap, which registers none of this.
