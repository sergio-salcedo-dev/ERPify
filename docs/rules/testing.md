...
- **When a port has several doubles, the pattern prefix stops discriminating and the name states the behaviour instead**, on the same axis as the port's production adapters: `FixedClock` / `AdvancingClock` / `MovableClock`, siblings of the production `SymfonyClock` adapter (and of the vendor's own `NativeClock` / `MonotonicClock` / `MockClock`). All three of those clocks are stubs, so `Stub` would name the category beside two names that name members, and the question a reader actually has — does the clock move, and who moves it — would go unanswered. This is not a carve-out for clocks: it is the rule the first row already states, applied where the pattern prefix carries no information.
...
## A test hands the aggregate its instant; the container's clock is pinned

Aggregates and domain events read no clock: the application layer reads its injected `Clock` once per operation and passes the `DateTimeImmutable` inward (ADR [`aggregate-receives-the-instant.md`](../adr/aggregate-receives-the-instant.md)). So a test decides every instant an aggregate carries, by what it passes:

- **A test that does not care what time it is** passes `Erpify\Tests\Double\Clock\SuiteInstant::now()` to a factory or mutator, or `SuiteInstant::clock()` to a use case. The mothers default to it.
- **A test that injects its own `FixedClock`** hands that same instant to every aggregate it builds for the scenario. An aggregate whose `createdAt` disagrees with the instant the test supplied is the defect this design removed — it produced a `Session` expiring 24 years before its own creation, in a green test.
- **A functional test** runs its request on the container's `clock` service, which delegates to Symfony's global clock. `FreezeSystemClockExtension::pin()` fixes that clock to `FreezeSystemClockExtension::SUITE_INSTANT` — the instant `SuiteInstant` returns — so a seed built with `SuiteInstant::now()` agrees with what the request reads. Override it with `Symfony\Component\Clock\Clock::set(new MockClock(…))` and seed the aggregates with the same instant; restore with `pin()`.
- **Alice fixtures** take theirs from `Erpify\Tests\DataFixtures\SeedInstant`, which reads Symfony's global clock: the wall clock under `make db.load.fixtures` and Behat, the pinned instant under PHPUnit.

`FixedClock` lives under `api/tests/Double/`, deliberately not under `api/tests/Support/`, whose namespace `ArtifactGateSweep` reads as a signal that a kernel-free test is an artifact gate.

`pin()` is called from three places, and the two subscribers are not enough on their own: `PreparationStarted` and `Finished` bracket each test, and `api/tools/phpunit/bootstrap.php` covers the windows no per-test event reaches — a data provider (resolved while the suite is *built*), `setUpBeforeClass()` of the first class, and an isolated child process, which registers no extension at all.

**Why pinning and not clearing.** Clearing hands the global clock back to the host wall clock, which is the failure mode itself: a test that seeds an absolute near-future expiry and reads it through an active-only predicate is green until that date arrives, then red on a commit that touched nothing. The tree shipped that twice. A pin does not make a bad seed correct — it makes its verdict the same on the first run and every run after.

**Free to a test is not free to a reader, and the constant carries two properties because of it.** It sits in a year the tree does not use — the value appears verbatim in failure diffs, and when it was `2026-01-01` (the tree's most-used date literal) that string led the reader to 65 files instead of to the constant, while making `assertSame('2026-01-01…', $x->createdAt)` true by two independent paths. And it sits away from every day/month/year boundary, so a test doing `->modify('-1 second')` does not cross all three at once. `2099` and `2100` are taken as the tree's idioms for "far future" and "locked for ever".

**Express a window, not a date.** `SessionMother` and `PasswordResetTokenMother` measure their default expiry from the instant the aggregate starts at (`startedAt`, `issuedAt`), mirroring the TTL production issues with, and each is pinned by a test that starts past any far-future literal a mother would plausibly carry.

**`api/src` has one time source, and that is gated** (`make php.lint.wall-clock`): a bare `new DateTimeImmutable()`, a relative literal, `time()`, `date($format)` or Symfony's global clock reached statically is refused there. It gates production code only; a test that fabricates a date literal stays a review matter.

**Blind spots.** `FreezeSystemClockExtensionTest` witnesses the pin but not its *leading* edge, which the trailing pin satisfies on its own; the case it defends (a test that skipped or threw in `setUp()` and so never reached `Finished`) cannot be staged in a suite that forbids ordering dependencies. Postgres keeps a clock nothing here touches, and Behat boots from its own bootstrap, which registers none of this.
