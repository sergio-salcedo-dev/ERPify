...
- `scheduler_worker` — **single-replica** consumer of the `scheduler_maintenance`, `scheduler_audit_maintenance` and `scheduler_identity_maintenance` transports (the daily `handled_domain_event` prune, the daily 30-day prune of the `failed` transport, and the hourly dead-letter check; the `audit_log` retention prune and crypto-shredding erasure reconciliation; the person-reference reconciliation). Symfony Scheduler derives ticks from an in-process clock, so it is isolated here to fire once; on the scaled `messenger_worker` pool it would emit one tick per replica. **Must stay at `replicas: 1`** — the lock-based single-pool alternative is tracked in #261. Every `#[AsSchedule]` owes its transport a slot in this command *and* in the dev `messenger_worker` one: the transport is created from the attribute and declared in no config, so a schedule nobody consumes ships dead with every check green — `make php.lint.schedule-consumption` is what refuses that.
