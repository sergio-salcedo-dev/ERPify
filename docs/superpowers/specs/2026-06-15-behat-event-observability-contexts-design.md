...
- [x] [Review][Defer] New `*property … should be null` / `should have :count elements` steps error (uncaught `evaluate()` throw) on an absent dot-path instead of asserting — pre-existing `JsonToolTrait`/`JsonInspector` `THROW_ON_INVALID_PROPERTY_PATH` semantic (`should not exist` correctly try/catches it). No current scenario hits it. [`api/tests/Behat/Context/{OutboxContext,MercureContext}.php`] — deferred, pre-existing trait behavior
- [x] [Review][Defer] Non-queue-qualified outbox steps (`messages()`) concatenate `async`+`failed` under one global 1-based index, and Mercure/Notification selection defaults to `?? array_key_last(...)` — both safe today (single event, count asserted first) but silently target the wrong/last record once a handler produces multiple updates/emails. [`api/tests/Behat/Context/{OutboxContext,MercureContext,NotificationContext}.php`] — deferred, latent multi-event hardening
