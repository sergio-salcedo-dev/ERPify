...
- **PII-aware presentation**: `ActorChip` reads the materialised `actorErased` GDPR flag as a distinct «anonimizado (GDPR)» chip (the post-erasure UUID is never shown as an id); the drawer suppresses the follow-this-actor **and** follow-this-resource pivots on an erased axis, symmetrically — the leftover identifier is a pseudonym, so pivoting on it would filter the trail by an identity that denotes nobody while presenting it as a live reference; `RedactedValue` renders the `[REDACTED]` sentinel (≠ a real null «—»); `MetadataBlock` escapes JSON with a size guard. The drawer's detail-only `ip`/`userAgent`/`metadata` sections are **specified-but-dormant** until the 4.2a detail endpoint lands (deferred until auth) — the drawer renders them from an optional `detail` prop, dormant otherwise.

UX contract: `_bmad-output/planning-artifacts/ux-designs/ux-ERPify-2026-06-26/` (`EXPERIENCE.md` / `DESIGN.md`) — the original run, whose `Jornada` toggle and `?view=` axis were retired; correlation reads as a per-row chip plus a filter axis.
