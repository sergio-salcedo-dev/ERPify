...
**No request hangs forever.** Every request carries an `AbortController` armed with a 30 s default; `RequestOptions.timeoutMs` overrides it per call. Giving up mints `request-timeout` (status 0) rather than `network-error` — an offline client sent nothing, while a timed-out request may well have been received and applied, and reading them as one thing misleads triage. The per-call override is what lets a caller state a tighter bound without owning a timer of its own: sign-out passes 3 s down through `SessionsRepository.revokeCurrent(budgetMs)`, a port that speaks in milliseconds so no transport type reaches it.
