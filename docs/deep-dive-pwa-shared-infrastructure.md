...
`shared/infrastructure` is the cross-cutting **outermost layer** of the PWA — the only place allowed to depend on Next, Inversify, `fetch`, and Shadcn UI primitives. It holds three orthogonal sub-systems every bounded context relies on:
...
- **Exports:**
  - `API_PREFIX_V1 = "/api/v1"` — `as const`, prefixes every v1 route.
  - `ApiRoutes` — `as const` nested record: `v1.frontoffice.health`, `v1.backoffice.health`.
- **Imports:** none.
- **Used by:** `frontoffice/health/infrastructure/ApiHealthCheckRepository.ts`, `backoffice/health/infrastructure/ApiHealthCheckRepository.ts`, `HttpClient/HttpClient.ts` (`MockHttpClient` uses it to discriminate canned responses).
- **Side effects:** none.
- **Contributor note:** Adding a new bounded-context endpoint (PRD epics 5+): add the path here first, then reference `ApiRoutes.v1.<bc>.<endpoint>` from the repository. Do **not** template URLs in adapters. Search `MockHttpClient` for the URL discriminator pattern; you must extend it too or mocks break silently in tests.
- **Risks:** `as const` narrows the literal types — referencing a route absent on the API is caught by TS, but adding a path here without the matching `routes.yaml` entry compiles fine and fails at runtime.

### 2.2 `DependencyInjection/Container.ts` — 33 LOC

- **Purpose:** Build & export the singleton Inversify `Container`. Binds `HttpClient` (Fetch or Mock by env) plus per-context use-case wiring (`FrontOfficeCheckHealth`, `BackOfficeCheckHealth`) and their repositories.
- **Exports:**
  - `container: Container` — pre-configured. Intended for `container.get<T>(token)` at the App Router entry-point boundary only.
...
- **Side effects:**
  - Reads `process.env.NODE_ENV` and `process.env.VITEST` at module-load → `MockHttpClient` in tests, `FetchHttpClient` elsewhere. **Frozen at first import**; toggling the env later in the same process has no effect.
...
- **Contributor note:** The container imports concrete classes from sibling contexts, so **`shared/infrastructure` depends on every other context's `infrastructure/` and `application/` layers** — it is the composition root. Add new bounded-context bindings *here*, not in the context's own folder. Use the `FrontOfficeXxx` / `BackOfficeXxx` import-rename pattern when symbols collide.
- **Risks:**
  - String tokens are **not type-checked at the binding ↔ inject seam**. A typo in `@inject("FrontOfficeHealthCheckRepository")` compiles and fails only at runtime. Consider `Symbol`-based identifiers (or a typed `TYPES` const map) before the container grows beyond ~10 bindings.
  - `BackOfficeCheckHealth` / `FrontOfficeCheckHealth` are bound *transient* → a new instance per `.get()`. Fine today (no state); watch for accidental in-flight caching if injected deeply.
  - The mock toggle relies on `process.env.VITEST === "true"`; a future runner not setting that flag → real HTTP in tests. Prefer overriding the binding at the test boundary if you switch runners.
- **Verification:** `make pwa.test.unit` (mock path); `make pwa.dev` then exercise `/` & `/backoffice/health` (fetch path).
...
- **Side effects:** Real network I/O via `fetch` (`FetchHttpClient`), `setTimeout` (`MockHttpClient`).
...
- **Contributor note:**
  - `get<T>` returns the **raw JSON-decoded body**. There is currently **no parsing of RFC 9457 `application/problem+json`** — `!res.ok` throws a generic `Error("HTTP <status>")`, dropping `correlation-id`, `instance`, and `type`. The API error contract requires consumers to surface those fields; until `HttpClient` parses Problem Details, every adapter must do it manually (today none does — see the `// TODO` in `app/backoffice/health/page.tsx`).
  - When adding `post<T>` / `put<T>` / `delete<T>`, do it on the same port and wire body serialisation + `Content-Type: application/json` here, not in callers.
- **Risks (must fix before non-trivial usage):**
  - **No Problem Details parsing.** Errors lose `correlation-id`, breaking the cross-stack debug story in [`api-error-contract.md`](./api-error-contract.md). High-priority cleanup.
  - **`cache: "no-store"` is unconditional.** Fine for health/dynamic data; fights Next.js's RSC fetch cache for any future read-mostly endpoint. Make it caller-controlled.
  - **No `Accept: application/problem+json`.** Add it once you parse Problem Details, else content negotiation may regress in unrelated upgrades.
...
- **Purpose:** App-wide animated button on `motion/react` (Framer Motion successor) with 7 variants × 5 sizes. Distinct from `components/ui/button.tsx` (raw Shadcn primitive) — this one adds motion + project tokens.
...
- **Used by:** `ui/components/molecules/FeatureCard.tsx`, `ui/components/organisms/Navbar.tsx`. (Not `components/erpify/*` — those use the Shadcn `Button` directly.)
- **Side effects:** none beyond motion.
- **BEM:** `btn`, `btn--primary|secondary|outline|emerald|slate|ghost|link`, `btn--sm|md|lg|xl|icon`.
- **Contributor note:** `loading={true}` swaps children with the literal string `"Loading..."` — no spinner. Add a spinner here so every consumer benefits. Leftover comment notes (lines 46–50) muse about wrapping Shadcn's button — clean those up next time you touch the file.
- **Risks:** Two `Button` components exist (this + `components/ui/button.tsx`) — easy to import the wrong one. `BackOfficeLayoutClient` and `health/page.tsx` use the Shadcn one; landing/`Navbar`/`FeatureCard` use this one. Watch for drift.
...
- **Hard-coded text:** `"Erpify"`. On a marketing rename, grep the literal — it appears here and in `Footer`.
- **Default `href`:** `"/"`. BackOffice consumers should pass `href="/backoffice"` to anchor the brand to the dashboard.
...
- **Contributor note:** `loading` and `onClick` forward to the inner `Button`. The `children` slot renders below the button (`feature-card__extra-content`) — used by `app/page.tsx` to show health-check status text after the CTA fires.

### 2.7 `ui/components/molecules/PlaceholderCard.tsx` — 30 LOC

- **Purpose:** "Empty/coming-soon" tile filling the back-office dashboard grid before real widgets exist.
...
- **Contributor note:** Distinct from `components/erpify/EmptyState.tsx` — this is a static dashboard tile; `EmptyState` is the canonical zero-data placeholder for data-bound views (lists, tables). Don't merge them.
...
- **Contributor note:** Only **one level of nesting** is supported (`subItems: SubItem[]`, no `subItems` on `SubItem`). For deeper trees, refactor `SidebarItem` to recurse on itself before bolting on `subSubItems`.
...
- **Contributor note:** `value` is `string`, not `number` — caller formats/localises. Before adding async logic here, consider a `<StatCard.Loading />` skeleton variant.
...
- **Contributor note:** `Logo` is rendered with custom `iconClassName` and `children` (a coloured pill wrapping `ShieldCheck`) — the canonical override pattern. The dropdown items wire to nothing; route them to real handlers when auth lands.
...
**Entry points (modules nothing else inside `shared/infrastructure` imports):** `Container.ts`, every UI organism (`Navbar`, `Footer`), `PlaceholderCard`, `SidebarItem`, `StatCard`, `FeatureCard` — all leaves except `ApiRoutes`, `Button`, `Logo`, `HttpClient`.

**Leaves (import nothing inside scope):** `ApiRoutes.ts`, `Button.tsx`, `Logo.tsx`, `Footer.tsx`, `PlaceholderCard.tsx`, `SidebarItem.tsx`, `StatCard.tsx` (all `*.tsx` UI files depend only on Shadcn/Lucide/motion + `@/lib/utils`).
...
`shared/infrastructure` has **no test coverage** (`tests/context/shared/infrastructure/` does not exist) — a gap; see §6.
...
- ⚠ `Container.ts` is the only place importing across contexts (`frontoffice/health` *and* `backoffice/health`). Intentional — it's the composition root — but document it loudly when onboarding.
...
- Caller sees a stringly-typed error, with no `correlation-id` to surface in the toast.

**Target (per [`api-error-contract.md`](./api-error-contract.md) and the live `// TODO` in `app/backoffice/health/page.tsx`):**
- On `!res.ok && content-type startsWith "application/problem+json"`, parse the body, guard with `isProblemDetails` (already in [`shared/domain/ProblemDetails.ts`](../pwa/src/context/shared/domain/ProblemDetails.ts)), throw a typed `ProblemError extends Error` carrying the envelope.
- Components route on `problem.type`, render `<ProblemDisplay>` (exists at `components/erpify/ProblemDisplay.tsx`), and surface `correlation-id` via `<CorrelationIdChip>`.
...
The container is **only resolved inside `app/`** — no deeper code calls `container.get`. This keeps the DI-resolution boundary tight.
...
| Mercure SSE | Symfony API at `/.well-known/mercure` | **Not yet wired in `shared/infrastructure`.** Architecture allows it; today no PWA code subscribes. Add an `EventSourceClient` here when needed. |
...
| A new card variant | Compare `FeatureCard` vs. `PlaceholderCard` vs. `StatCard` first to pick the right base; extract a shared `<Card.Frame>` if you copy their `Card className="…rounded-2xl shadow-sm"` chain a third time. |
| A new error renderer | `components/erpify/ProblemDisplay.tsx` and `AsyncBoundary.tsx` exist — the canonical RFC 9457 UI surface. Wire `HttpClient` to a typed `ProblemError` rather than rebuilding the renderer. |
...
- **Re-export with rename to disambiguate cross-context symbols** (`Container.ts`):
  ...
  ```
- **String DI tokens namespaced by `<Office><Concept>`** — `FrontOfficeHealthCheckRepository`, `BackOfficeCheckHealth`. Keep until the token soup justifies a `TYPES` map.
...
- **Env-toggled adapter binding** — `MockHttpClient` for tests, `FetchHttpClient` otherwise. Copy this shape for a new outbound integration (e.g. an analytics SDK that should be a no-op in tests).
...
- [ ] **Did you touch `Container.ts`?** Re-run `make pwa.dev` and exercise `/`, `/backoffice`, `/backoffice/health` — wiring failures don't surface in unit tests today.
...
- [ ] **Did you touch a UI component?** BEM names must follow `block__element--modifier`. Verify the consumer renders unchanged in a real browser — Vitest won't catch motion / layout regressions.
...
- [ ] **Did you remove a binding?** Grep `@inject("…")` for the token across all contexts before deleting — TS won't catch a stale string.
- [ ] **Run `make pwa.quality` before pushing.**

## 9. Known follow-ups (file an issue if you don't do them now)

1. **Parse RFC 9457 in `HttpClient`.** Replace the generic `Error` with a `ProblemError` carrying the envelope. Wire `app/backoffice/health/page.tsx` to drop its synthesised fallback.
...
6. **Decide between this `Button` and `components/ui/button.tsx`.** Consolidate, or document the use-case.
