...
The `event_store` axis. All three live in **Backoffice / Bank** (`aggregateType: Backoffice.Bank`),
are recorded inside the `Bank` aggregate, and are published by their use case through the `EventBus`
port inside the write transaction (atomic with the aggregate row — no dual-write). All route to the
**`async`** transport (Doctrine outbox → `messenger_worker`, at-least-once).
...
A **separate contract** from the stored payload, owned by the reactor
[`RefreshRealtimeOnBankChanged`](../../api/src/Backoffice/Bank/Infrastructure/Messenger/RefreshRealtimeOnBankChanged.php).
Every `Update` is published **private** (`private = true`). The wire shape drops logo / stored-object
metadata and adds `accountCount`, resolved at publish time via `BankAccountCountEnricher::countFor()`.
...
Delivery is at-least-once; clients reconcile by `id`, so a re-applied update or delete is a no-op.
