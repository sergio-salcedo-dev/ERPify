...
Irreversible means *persisted, or a public contract* — not merely expensive to refactor. By that test two decisions cannot be postponed. The first: **the domain never knows a physical storage key, and image bytes are not part of the relational persistence model.** Aggregates reference a stable `ImageId`; the bytes live outside the relational database behind an infrastructure storage port, and where they live is nobody's business upstream. Stating the negative half matters because it is the half a future reader could otherwise satisfy with a `bytea` column: keeping binaries out of the relational model is what lets database growth stay independent of asset growth and image delivery evolve independently of the application. **Which storage technology serves them is deliberately deferred** — a local filesystem, an S3-compatible object store or anything else is an infrastructure adapter, reversible by construction, and so fails this section's own irreversibility test; it belongs to deployment, not to this contract. The second is the pipeline's edge, argued below and pinned as invariant 6 — a producer that is not an upload would otherwise have to enter behind the pipeline instead of through it, which is the redesign this section promises no deferred item forces.
...
**The read path belongs to the slice, not to a later one.** An opaque identifier is unguessable, which is not the same as authorised, and serving an avatar is a read of a personal record — a case [`regulatory-audit-trail.md`](./regulatory-audit-trail.md) D4 already brought under "every read of every entity" with a resource extractor. The story that serves bytes declares the voter it expects, or declares the route consciously public and argues it, and the read is audited like any other. **The first slice answers this without a consumer, and narrows both halves:** authentication as a provisional frontier and no audit row until a consumer classifies its images — [`image-read-route-contract.md`](./image-read-route-contract.md) D1–D2.
...
- **`Documents` gets its own storage port; there is no generic one spanning both.** The two contracts disagree on the port's *shape*, not merely on its backend: evidence is written as a stream and sealed (the missing stream-oriented encryptor above is that same constraint arriving from the crypto side), while `ImageStorage` promises a retrievable blob and a reliable delete over content it is free to have transformed. The withdrawn implementation shared a single `StoragePort` across both concerns. That coupling is what blurred the boundary — a port that serves both makes "it was the only door" (D2) look reasonable at the call site — which is why two independent contracts are required here rather than one generic storage abstraction.
...
- The derivative-of-evidence leak in D5 is a privacy defect by construction, so any story touching erasure or derivative lifecycle needs a recorded adversarial pass, and self-certification does not close it. **This is a context-local requirement as of 2026-08-31**: the root `CLAUDE.md` retired its own requirement that day and now only recommends the pass, so this ADR is no longer restating a rule from above — it is keeping a stronger one for the privacy-by-construction axis, which is a deliberate exception and is stated as such rather than inherited.
