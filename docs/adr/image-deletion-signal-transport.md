...
**What actually provides the after-commit guarantee is the mechanism, not the transport's name.** `async` resolves to the Doctrine transport over the same connection, and `EventBus::publish()` is called inside `transactional(...)`, so the `INSERT` into `messenger_messages` lives in the same transaction and a rollback takes it with it — the worker cannot see an uncommitted row. **The property depends on the DSN.** Point `MESSENGER_TRANSPORT_DSN` at an external broker and the publish leaves the transaction, the guarantee disappears, and every gate here stays green. **No test in this repository asserts that guarantee, and saying otherwise would point a reader away from the risk rather than at it.** The subject of such a test is a real publisher, which this story does not have — `new ImageDeletionRequested` appears nowhere in `api/src` — and the test environment substitutes `async` with `in-memory://`, a transport that does not join the transaction at all (measured: it retains the message after a rollback). What IS asserted is narrower and true: that the event reaches a transport (`PersistentTransportPolicyGateTest::everyAdrExceptedEventActuallyReachesATransport`, red when the routing line is deleted) and that the transport's DSN still resolves to Doctrine outside test (`PersistentTransportRoutingShapeGateTest::theDurableTransportStillHasATransactionalDsn`, red when it is rewritten). The end-to-end observation belongs to the story that introduces the first real publisher, in the consuming epic — @accepted-risk #872.
...
- **And when that second window closes, what is lost is the INSTRUCTION, not just the identifier.** The prune's own rationale — *the domain event survives, only the failure context goes with the row* — holds for an event that is a fact with replayable projections. It does not hold here: `DeleteImageOnDeletionRequested` is an `#[AsMessageHandler]`, not a `Projector`, so nothing re-dispatches the request from `event_store`, and this module keeps no bookkeeping that could reconstruct it. The state left behind is bytes on disk, a live `image` row, and no pending request. Reaching it needs a storage outage that outlives the retry budget, which is why that budget is now declared on the transport rather than inherited: Symfony's default gave it seven seconds. Recorded as residual two of `PRODUCTION_SECURITY_CHECKLIST.md` §7, which is where the consuming epic reads it.
