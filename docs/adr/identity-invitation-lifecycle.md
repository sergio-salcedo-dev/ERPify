...
Cut as its own epic with `bmad-create-epics-and-stories`; the scoped architecture addendum that localized each decision to a PR and gave the dependency DAG landed with this ADR in PR #458 (commit `4ccb2f92`) and is kept by git. Bootstrap of the first organization + first admin stays CLI-only; credentials are never seeded in migrations; dev/test via Alice fixtures.
