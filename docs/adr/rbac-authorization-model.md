...
The load-bearing frozen invariant it must respect is that **roles are external authorization policy** — no `Application`/`Domain` code branches on a role: [`auth-rbac-subsystem.md`](./auth-rbac-subsystem.md) D3.
