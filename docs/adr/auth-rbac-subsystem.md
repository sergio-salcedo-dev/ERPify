...
- **CSRF** (D1): el **login** queda cubierto por same-origin + CORS no ampliada + requisito `application/json` + `SameSite=Lax`; la protección por **token stateless para rutas mutantes** se cablea con la **primera ruta autenticada mutante** (`json_login` no valida token — *wire-on-consumer*), no antes.
...
- **CSRF del login: same-origin + CORS no ampliada + requisito `application/json` + `SameSite=Lax`** (D1, enmendado en AF-1.2 al descubrir que `json_login` no valida token); el token *stateless double-submit* se cablea con la 1ª ruta autenticada mutante (*wire-on-consumer*).
