...
| **`scheduler_worker`** | Single-replica **`messenger:consume scheduler_maintenance scheduler_audit_maintenance scheduler_identity_maintenance`** process: the `handled_domain_event` prune, the 30-day retention prune of the `failed` transport and the hourly dead-letter check, the `audit_log` retention prune and erasure reconciliation, the person-reference reconciliation, the session sweep and the five-minute lockout notice. Split off `messenger_worker` so the in-process Scheduler clock fires once, not once per replica. **Keep at `replicas: 1`** (lock-based alternative: #261). Adding an `#[AsSchedule]` means adding its `scheduler_*` transport here — `make php.lint.schedule-consumption` fails when one is missing. |
