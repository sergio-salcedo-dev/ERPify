...
| **`scheduler_worker`** | Single-replica **`messenger:consume scheduler_maintenance scheduler_audit_maintenance scheduler_identity_maintenance`** process: the `handled_domain_event` prune and dead-letter check, the `audit_log` retention prune and erasure reconciliation, and the person-reference reconciliation. Split off `messenger_worker` so the in-process Scheduler clock fires once, not once per replica. **Keep at `replicas: 1`** (lock-based alternative: #261). Adding an `#[AsSchedule]` means adding its `scheduler_*` transport here — `make php.lint.schedule-consumption` fails when one is missing. |
