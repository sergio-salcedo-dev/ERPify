...
| **`scheduler_worker`** | Single-replica **`messenger:consume scheduler_maintenance scheduler_audit_maintenance scheduler_identity_maintenance`** process: **nine** ticks — the `handled_domain_event` prune, the 30-day retention prune of the `failed` transport and the hourly dead-letter check, the `audit_log` retention prune and erasure reconciliation, the person-reference reconciliation, the stored-identity inspection, the session sweep and the five-minute lockout notice. Split off `messenger_worker` so the in-process Scheduler clock fires once, not once per replica. **Keep at `replicas: 1`** — gated by `make php.lint.schedule-consumption` for every root compose file, though only for what the repository declares: `--scale` overrides it at the command line. Lock-based alternative declined in #261. Adding an `#[AsSchedule]` means adding its `scheduler_*` transport here — `make php.lint.schedule-consumption` fails when one is missing. |
...
| **`MAILER_SMTP_TIMEOUT`**        | Seconds an `smtp://`/`smtps://` server may hold a caller on **one socket operation** — connect, read and STARTTLS alike. **Not a bound on a send:** SMTP is a conversation, so a send costs round-trips × this value (measured: a relay answering every command after 9s held one send for 54s at a 10s bound). Defaults to `3` so that product stays under the 60s php.ini `default_socket_timeout` it replaces; accepted range is `1`–`300` and `make prod.env.check` rejects anything else before the stack starts. It has a compose default on all three PHP services, so removing it from `.env.prod.local` changes nothing. `smtp://host?timeout=N` overrides it per DSN. Other schemes (API bridges, `sendmail://`) carry their own timeouts and ignore this. |
| **`DEFAULT_NOTIFICATION_EMAIL`** | Operational inbox for bank create/update notifications (async handler).                                                                                                                                                                                                                                                                               |

`compose.prod.yaml` passes **`APP_SECRET`** and **`MAILER_DSN`** into **`messenger_worker`**; other worker variables come from the base **`compose.yaml`** merge (e.g. **`DATABASE_URL`**, **`MESSENGER_TRANSPORT_DSN`**, **`DEFAULT_NOTIFICATION_EMAIL`**). If you add env-only overrides in production, ensure **all three** PHP services stay aligned — **`php`**, **`messenger_worker`** and **`scheduler_worker`**. The third is the one to check: it is declared only in `compose.prod.yaml` and inherits nothing from the base merge, so every mailer variable it needs must be repeated on it by hand. It is also the service whose failures are quietest — the lockout notice is best-effort, so a broken mailer there logs a warning and returns, and nothing else contradicts it.
