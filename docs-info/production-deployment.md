...
| **`MAILER_SMTP_TIMEOUT`**        | Seconds an `smtp://`/`smtps://` server may hold a caller, connect and read alike. Defaults to `10`; unset it and the socket falls back to php.ini's `default_socket_timeout` (**60s**) inside a request and inside the single-replica lockout-notice tick. `smtp://host?timeout=N` overrides it per DSN. Other schemes (API bridges, `sendmail://`) carry their own timeouts and ignore this. |
