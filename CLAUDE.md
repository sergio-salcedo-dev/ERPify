...
- **Claiming a version in `docs/project-context.md`** → bind it in `api/.project-context-versions`. That page is not the lightly-read note its history suggests: **60 of the 90** installed skills declare it in `persistent_facts`, so it is loaded as foundational context at the start of an agent session rather than consulted on demand, and a stale line is a false premise delivered before the agent reads any code, asserted with exactly the confidence of a true one. The page restates normative rules as well, and **that half stays there**: a measured attempt to cut it to versions-and-pointers left ~24 rules existing nowhere else in the repo and deleted the only refutation of a live falsehood five other documents assert as fact (that something scans a commit — there is no `.pre-commit-config.yaml`, no installed hook, no `hooksPath`). What is gated is the half a cheap check can falsify. Measured over the page's history: ten drifted claims, **all** of them prose, against **zero** wrong version numbers — so the gate covers the direction that has never failed, and review still owns the one that has. Gate: `make php.lint.project-context`, three directions — the manifest still starts with the claimed version, the claimed **token** still appears on the page, and every version the page's tables state is bound by a line (33 claims, extracted from the second column; the completeness half is why adding a row without a line now fails instead of passing in silence). The token carries its product name (`Behat 4`, not `4`) or the staleness direction is a tautology, and only a constraint with a **floor** is read — `<2.0` and `>2` used to satisfy "2", and `^24.15.0 || >=26.0.0` used to satisfy "Node 24" on a container running 26. A claim nothing can own is declared `unbound :: <reason> => <token>` rather than omitted, so an exemption never looks like an oversight. A green proves nothing about the tables' prose column, nothing about the prose outside the tables (most of the page), nothing about a version stated in a paragraph rather than a cell, nothing about an `unbound` reason — which is a statement, not a proof — and nothing about a digest.
