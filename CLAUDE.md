...
- **Queuing an event about a person** → never. An "aggregate id alone" payload is safe on a persisted transport **iff** the aggregate is not a natural person: `async`/`failed` are Doctrine tables with no TTL and no prune that no erasure path touches, so a queued person id outlives the erasure the app confirmed to the subject. Classify every `aggregateType()` in `api/.persistent-transport-policy`; leave person-aggregate events unrouted (handled in-process) and put blocking or failing effects in the use case post-commit, not in a handler. Gate: `make php.lint.persistent-transport`.
