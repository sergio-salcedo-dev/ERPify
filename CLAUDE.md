...
- **Claiming a version in `docs/project-context.md`** → bind it in `api/.project-context-versions`. That page is not the lightly-read note its history suggests: 60 of the installed BMAD skills name it in `persistent_facts`, so it is loaded as foundational context at the start of an agent session rather than consulted on demand, and a stale line is a false premise delivered before the agent reads any code. The page was cut to versions, training-cutoff traps and pointers for exactly this reason — every normative rule it used to restate now lives only in the doc that owns it, because a second unversioned copy went stale faster than the original and the page's own instruction on a conflict was to ask the reader to resolve it. Measured over its history: ten drifted claims, all behavioural prose, against **zero** wrong version numbers — so the numbers are gated and the prose was moved rather than gated in place. Gate: `make php.lint.project-context`, both directions (manifest drift, and a token that left the page). A green proves no *registered* number contradicts its manifest; it proves nothing about the prose column, nothing about a version carrying no registry line, and nothing about a digest.
