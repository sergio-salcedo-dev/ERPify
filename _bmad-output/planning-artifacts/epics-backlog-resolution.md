...
| **Vigilancias automatizadas** | ~~#593~~, #196, #420 | Disparadores, no tareas. **#593 disparó el 2026-08-11** (upstream tagueó `v0.4.3` y `v0.5.0`) y se resolvió el 2026-08-12: el pin retirado, su vigía borrado con él. #196 no es una vigilancia sino un aplazamiento consciente (`sourcemaps: { disable: true }`, falta `SENTRY_AUTH_TOKEN`); #420 es un tripwire humano («al tercer VO string») y **ya se cumple**: `Email`, `SessionId` y `HashedPassword` comparten forma y no existe trait |
