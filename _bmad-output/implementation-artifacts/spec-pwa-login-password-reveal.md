...
| Gate detects | A `type` attribute, or a `type` property of an object literal written inside a spread, whose value subtree holds the literal `password` in any case: `type="password"`, `type="PASSWORD"`, `type={"password"}`, `` type={`password`} ``, `type={c ? "text" : "password"}`, `type={c && "password"}`, `{...{ type: "password" }}`, `{...(c && { type: "password" })}` | Fails, listing every offending `file:line` | N/A |
| Gate ignores | `const password = "password"`, a commented-out attribute, `type={type}`, `type="text"`, a spread of a named object (`const p = { type: "password" }; {...p}`), a concatenated value (`{"pass" + "word"}`) | No finding | N/A |
| Gate universe | A subtree skipped inside `walk()` | Fails against the runtime's own recursive enumeration, which shares no traversal logic | N/A |
| Gate universe | `SRC_ROOT` moved, narrowing both sides at once | Fails the file floor and the sentinel, which the comparison above cannot catch | N/A |
| Owner gone | `PasswordInput.tsx` deleted or stripped of its password type | Fails the "at least one declaration" assertion | N/A |
| Owner moved | `PasswordInput.tsx` renamed or moved | Passes that one and fails the ownership assertion, naming `OWNER` as the remedy | N/A |
...
- **Trigger:** a second external review of the implementation (twelve points). **Amended:** the spread universe is now uniform and stated exactly (any object literal written inside the spread expression, at any depth; a named variable still is not); paths are compared in one POSIX spelling; the universe assertion names a concrete sentinel file instead of any file under `src/app`; fixtures gained `type="PASSWORD"` and a guarded spread; the component's comment leads with the contract rather than the consequence; the text-assist override is asserted for all three attributes; the re-mask is asserted on the form's `submit` **event** rather than only a button click; the round-trip carries a password with quotes, a backslash, angle brackets and an accent; and the field/form counts are worded unambiguously. **Known-bad state avoided:** a `walk()` skipping `src/app` clears the 300-file floor at 303 files — measured — so without the sentinel that narrowing passes. **KEEP:** the node-identity pin, and the refusal to widen the gate to `secret`/`token`, variables or `React.createElement`.
- **Trigger:** a third, adversarial review (F1 HIGH, F2 MEDIUM, D1 LOW). **Amended:** the universe is now *identified* rather than counted — the walked set is compared against `readdirSync(..., { recursive: true })`, an enumeration sharing no traversal logic with `walk()`, because a floor plus a sentinel cannot see a single skipped subtree (F1); the listener's lifecycle is falsified by asserting the handler removed is the one added and that a remount re-subscribes to the new form (F2); and the I/O matrix's gate rows were rewritten (D1) — **this edit reaches inside `<frozen-after-approval>`**, done deliberately because the rows were a stale statement of a contract a future reviewer would read as authoritative, and because the correction is factual rather than a change of intent. **Known-bad state avoided:** a `walk()` skipping `src/components/erpify` clears the floor and keeps the sentinel, so it was green before this. **KEEP:** the floor and the sentinel — they catch a moved `SRC_ROOT`, which the new comparison cannot, since it narrows both sides at once.

## Design Notes

The default flip is a no-op today and that is the point of doing it now: **seven password fields across six forms** — `ChangePasswordForm` carries two — pass the prop explicitly, so no rendered behaviour moves — only the direction the *next* omission fails in. Counted against the tree, not assumed.
...
- Falsifications, each restored by copying the backed-up bytes (never `git checkout`): plant a password input in a second file (gate names it); strip the owner's password type (non-empty assertion); rename the owner (ownership assertion, not the non-empty one); narrow `SRC_ROOT` to `src/components`, and separately skip `src/app` inside `walk()` (universe assertion — the second clears the file floor at 303 and is caught only by the sentinel); plant `type="PASSWORD"` and a guarded inline spread in a real file (both named with their line); duplicate a `toggleTestId`; drop `{...register("password")}`; delete the submit listener (exactly the three re-mask cases fail); corrupt only the submitted value with `.slice(0, 4)` (exactly the round-trip case fails).
