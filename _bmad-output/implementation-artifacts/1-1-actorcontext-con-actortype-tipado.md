...
`enum ActorType: string` con backing `'anonymous'|'system'|'api_key'|'user'` — los tokens **exactos** del esquema del ADR (`actor_type enum anonymous|system|api_key|user`). Es una **divergencia consciente** del precedente `BankAccountStatus`/`SortDirection` (backing en MAYÚSCULA): aquí los valores son el contrato que persistirá la Story 1.3 en una columna `VARCHAR` vía `$actorType->value` (NO un enum nativo Postgres ni `EnumType` —que es una constraint del Symfony Validator, no un Doctrine Type—; la validación del conjunto cerrado la da el propio tipo enum PHP), y el ADR los fija en minúscula. El detalle de la columna vive en la Story 1.3. No cambiar a mayúscula.
