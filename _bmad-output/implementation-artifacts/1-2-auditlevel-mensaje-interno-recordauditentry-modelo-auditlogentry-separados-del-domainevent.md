...
**AC5 — `action` no vacío y acotado en longitud; `resourceType` acotado (invariantes del registro).**
**Given** `AuditLogEntry::create(...)` con `action` vacío (cadena vacía o solo espacios), o con `action` de longitud > 100, o con `resourceType` no nulo de longitud > 100,
**When** se construye,
...
**And** con un `action` no vacío de longitud ≤ 100 y `resourceType` `null` o de longitud ≤ 100 se acepta;
**And** el límite de 100 refleja el ancho de columna `VARCHAR(100)` de `action`/`resource_type` definido en 1.3 (acoplamiento longitud-dominio ↔ longitud-columna): así un `action`/`resourceType` largo de un futuro callsite falla en construcción (accionable, fluye a Sentry) y **no** en el `INSERT` de Postgres.
