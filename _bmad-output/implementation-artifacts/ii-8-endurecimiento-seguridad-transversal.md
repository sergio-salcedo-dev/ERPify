...
10. **Observaciones elevadas a Sergio — las 3 RESUELTAS (2026-07-14, AskUserQuestion):** (a) copy del muro → **description por ruta** ratificada e implementada en este PR: variante `INVALID_RESET_LINK` con título/status/motivo byte-idénticos a `INVALID_LINK` (la opacidad protege el POR QUÉ murió el enlace, no el flujo — la URL ya lo nombra) y solo la salida cambia (description «Puedes solicitar un enlace nuevo desde "¿Has olvidado tu contraseña?"» + acción secundaria `Solicitar enlace nuevo` → `/forgot-password`; la invitación sigue apuntando al administrador); (b) `iam_session` post-erasure → **issue #495** (servicio Session-owned + enganche en `EraseIdentitySubject`, seam espejo de `RevokeSessionsBestEffort`); (c) buzón de seguridad → **Gmail del operador como valor provisional de prod** documentado en `api/docs/production-ready/secrets.md` (mismo buzón monitorizado que `DEFAULT_NOTIFICATION_EMAIL`, ya presente en el repo) hasta que exista `seguridad@…`.
...
`api/src/Shared/Mailer/Infrastructure/{BulletproofEmailChrome,SecuritySenderAddress,SecurityMailerMisconfigured}.php`
...
`pwa/src/context/shared/error/infrastructure/ui/AccessWall.tsx` (variante `INVALID_RESET_LINK`) ·
`pwa/tests/app/(auth)/{tokenActionScreen,resetPasswordForm}.test.tsx` ·
`pwa/tests/context/shared/error/infrastructure/ui/AccessWall.test.tsx` · `pwa/tests/next-config-headers.test.ts` (nuevo)
...
| 2026-07-14  | **PR #494 abierto. Las 3 observaciones resueltas por Sergio (AskUserQuestion):** (a) muro por ruta → variante `INVALID_RESET_LINK` implementada (título opaco idéntico, salida self-service a `/forgot-password`); tests actualizados (pwa unit 1049, quality verde); (b) `iam_session` post-erasure → issue #495; (c) `MAILER_SECURITY_FROM` provisional = Gmail del operador, documentado en `secrets.md`. |
| 2026-07-14  | **Sonar PR #494 (gate OK, coverage 94.1%, dup 0.0%) — 3 issues arreglados:** S112 ×2 → excepción dedicada `SecurityMailerMisconfigured` (factorías `nonHttpsBaseUrl`/`unmonitoredSender`, marker-less → espejo de `InvitedIdentityUnavailable` de II-4; tests migrados + `#[CoversClass]` para new_coverage); S1142 (4 returns en `RequestPasswordReset::request`) → guard `$issued` plegado a if positivo (3 returns). stan/quality EXIT=0/suite completa verdes (1 flake pre-existente del planner en `AuditTimelineSearchCursorFunctionalTest`, pasa solo y en re-run). | |
