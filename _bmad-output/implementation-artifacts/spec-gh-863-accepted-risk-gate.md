...
**Not resolved, called out for the human before merge:** the live-state workflow's actual behavior against the GitHub API remains unverified in every session that touched this branch -- neither the local `gh` CLI (broken keyring token) nor the GitHub MCP server (expired token) could authenticate. Confirm on the real PR that (a) `#860`/`#861` both report OPEN, (b) a deliberately-closed test issue reports a policy failure, and (c) `gh issue view <closed-id> --repo sergio-salcedo-dev/ERPify --json state` behaves as the script assumes.
