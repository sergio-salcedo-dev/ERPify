...
_Generated by `/bmad-code-review` on 2026-05-07. 3 review layers ran: Blind Hunter (diff-only), Edge Case Hunter (project read access), Acceptance Auditor (spec compliance). 1 decision-needed, 9 patches, 3 deferred, ~14 dismissed as noise._

_Second pass on 2026-05-07: re-ran the 3 review layers after patches landed. Acceptance Auditor confirmed all 18 ACs Met. One additional patch surfaced (duplicate-violations test) and applied. Verification: `make php.stan` 0 errors / 175 files; `make php.unit c='--filter=ProblemDetailsFactoryTest'` 68 tests / 309 assertions green; `make php.behat` 42 scenarios / 263 steps green; `make php.lint` clean._

- [x] **[Review][Patch] Wrapped `ValidationFailedException` from `MapRequestPayload` / `MapQueryString` on non-search routes loses `violations[]`** (resolution: walk `getPrevious()` chain in the new branch — mirror `SearchExceptionListener::findInChain`) — Symfony's `RequestPayloadValueResolver` wraps `ValidationFailedException` in `HttpException::fromStatusCode(422, …, previous: $vfe)`. `ProblemDetailsFactory::fromThrowable()` does bare `instanceof ValidationFailedException` (top-level only, no `$e->getPrevious()` walk), so for non-search routes (`BankPostController`, `BankPutController` use `#[MapRequestPayload]`) the exception falls through to the `HttpExceptionInterface` branch (line 134) → `type: invariant-violation`, NO `violations[]`, AND `title` ≈ `ConstraintViolationList::__toString()` (concatenated property paths + messages + codes — leaks per AC #4). Contradicts the `SearchExceptionListener` docblock at lines 28–31 which promises non-search wrapped VFE flows through `ExceptionResponder` "producing the unified RFC 9457 Problem Details body with the `violations` extension". `SearchExceptionListener` itself uses `findInChain($throwable, ValidationFailedException::class)` for the search path — the asymmetry is the bug. Production-affecting (Bank POST/PUT). Decide: extend AC #1 to walk `getPrevious()` chain in the new branch (one-line `findInChain` helper, mirrors the search listener), defer to a follow-up story (Epic 4 Story 4-3), or downgrade the docblock promise.
- [x] **[Review][Patch] Add `'violations'` to `RESERVED_KEYS`** [`api/src/Shared/Application/Problem/ProblemDetailsFactory.php:72`] — `RESERVED_KEYS = ['type','title','status','detail','instance','correlation-id']` does NOT include `'violations'`. A `DomainException::context()` carrying `'violations' => …` passes the reserved-key filter (line 225–226) and surfaces at the JSON top level pretending to be the structured Story 1.6 extension. PWA's `type`-only routing (FR44) cannot disambiguate. Add `'violations'` to the constant.
- [x] **[Review][Patch] Add unit regression test for class implementing both `DomainException` and `ValidationFailedException`** [`api/tests/Unit/Shared/Application/Problem/ProblemDetailsFactoryTest.php`] — AC #2 explicitly calls out this precedence (DomainException branch wins). Existing tests pin `DomainException` vs `HttpExceptionInterface` only. Add a test that constructs an anonymous class extending both and asserts `type` is the marker default (e.g., `invariant-violation`) AND `extensions` has no `violations` key.
- [x] **[Review][Patch] Add unit test for nested propertyPath verbatim preservation** [`api/tests/Unit/Shared/Application/Problem/ProblemDetailsFactoryTest.php`] — AC #5 says `field` is `getPropertyPath()` verbatim. Existing tests cover empty string only. Add a violation with `propertyPath: 'addresses[0].street'` and assert `field === 'addresses[0].street'` (no escaping, no collapsing of brackets/dots).
- [x] **[Review][Patch] `testValidationFailedExceptionViolationCodeFallsBackToEmptyStringOnNull` should explicitly pass `code: null`** [`api/tests/Unit/Shared/Application/Problem/ProblemDetailsFactoryTest.php`] — currently relies on Symfony's default constructor arg. If a future Symfony version changes the default to `''`, the test silently keeps passing while no longer exercising the null-coalesce path. Pass `code: null` as a named argument.
- [x] **[Review][Patch] Add unit test pinning `code: ''` (empty) survives without null-coalesce** [`api/tests/Unit/Shared/Application/Problem/ProblemDetailsFactoryTest.php`] — AC #5 collapses `null` → `''`, but no test asserts a violation explicitly carrying `''` stays `''` (vs accidentally going to `null` somewhere). Add a one-line companion test to `testValidationFailedExceptionViolationCodeFallsBackToEmptyStringOnNull`.
- [x] **[Review][Patch] Empty-list Behat scenario should assert `Cache-Control: no-store`** [`api/features/shared/error_contract/validation_violations.feature`] — only the first scenario asserts the cache header. Empty-violations 422 path could regress its header without the suite catching it.
- [x] **[Review][Patch] Empty-list Behat scenario could pass on JSON object form** [`api/features/shared/error_contract/validation_violations.feature`] — `the JSON node "violations" should have 0 elements` will pass for both `[]` and `{}` (count of an empty stdClass is 0). Add a raw substring assertion `the response body should contain "\"violations\":[]"` to pin the JSON-array shape end-to-end (parity with AC #6's footgun pin at the unit-test layer).
- [x] **[Review][Patch] Sensitive-payload Behat: add structural assertion that violation entries only have `field`, `message`, `code`** [`api/features/shared/error_contract/validation_violations.feature`] — current scenario asserts only that `'super-secret-payload'` and `'leaked-secret'` literals are absent. If a future regression adds a `value` or `template` key per violation, the literal-only assertions miss it. Add explicit absence of `"invalidValue"`, `"messageTemplate"`, `"parameters"`, `"cause"`, `"constraint"`.
- [x] **[Review][Patch] InvariantViolation Behat scenario `should not contain "violations"` is overly broad** [`api/features/shared/error_contract/validation_violations.feature`] — substring `"violations"` could false-positive (e.g., a future detail string mentioning the word). Tighten to `"violations":` (with colon) so the assertion targets the JSON key.
- [x] **[Review][Patch] Pin duplicate violations on the same field are preserved (no silent dedup)** [`api/tests/Unit/Shared/Application/Problem/ProblemDetailsFactoryTest.php`] (second-pass) — surfaced by the second-pass Edge Case Hunter. Added `testValidationFailedExceptionPreservesDuplicateViolationsOnSameField` (two violations on `'name'`, distinct messages and codes, both surface — no silent dedup). Closes the future-regression vector where someone adds dedup logic.
- [x] **[Review][Defer] Stringable `__toString()` throwing in `(string) $violation->getMessage()` and non-UTF-8 byte sequences** [`api/src/Shared/Application/Problem/ProblemDetailsFactory.php:343`] — deferred to Story 3.4 (last-resort static body on listener self-failure). A throwing `Stringable` or non-UTF-8 message bubbles out of the listener / breaks `JSON_THROW_ON_ERROR` in `ProblemDetailsResponder`, collapsing the response to a generic 500. Story 3.4's listener-self-failure wrapper is the right home.
- [x] **[Review][Defer] `isWhitelistedValue` has no depth/size cap on extension array values** [`api/src/Shared/Application/Problem/ProblemDetailsFactory.php:420-426`] — deferred to Story 3.6 (16 KiB body cap with truncation marker). DoS surface for any path that reflects user input through `context()`.
- [x] **[Review][Defer] `SearchExceptionListener` carve-out is asymmetric — `NotEncodableValueException` still produces legacy JSON:API on non-search routes** [`api/src/Shared/Infrastructure/Http/EventListener/SearchExceptionListener.php:46-55`] — pre-existing, documented carve-out. Out of scope for Story 1.6 (which scoped only the `ValidationFailedException` branch). Will be addressed in Epic 4 Story 4-3 (api integration sweep) per the closing commit message.

_Dismissed as noise (~14): PII via propertyPath (spec contract per AC #5); duplicate functional-test coverage (Behat covers); listener-priority test (Behat exercises the actual stack); `should not contain "leaked-secret"` substring narrowness (adequate for the intent); copy-paste between fixtures (style); mixed positional/named ConstraintViolation args (style); rfc-9457 schema doesn't constrain extension shape (intentional `additionalProperties: true`); no JSON_PRETTY_PRINT tolerance in regex (no caller enables it); `ThrowInvariantViolationDomainExceptionController` empty-type arg (intentional fixture targeting marker-default); Iterator pointer rewind (Symfony default impl is `IteratorAggregate`, fresh iterator each time); wire array shape checked via regex only (adequately pinned at multiple layers); no functional-test wire-key-order pin (unit test pins it); Behat scenario 5 weaker than spec's prescribed fallback (documented in Completion Notes — Behat 3.31 colon-placeholder collision; unit test pins byte order); `firstMatchingMarker` reliance on PHP `class_implements()` order (pre-existing, not 1.6 scope)._
