...
- [x] **H1. Count query ignores cursor `andWhere` → wrong totals beyond page 1** — `Paginator.php:88-97`
...
- [x] **H2. `hasMorePages` always `false` in `DETAILED` mode** — `Paginator.php:67-79, 127-134`
...
- [x] **H3. Cursor never returned to the client** — `BanksGetController.php:33-42`
...
- [x] **H4. Invalid `paginationMode` silently downgrades to DETAILED** — `BanksGetController.php:23-26`
...
- [x] **H5. Unbounded `page` parameter — OFFSET DoS** — `BanksGetController.php:26`
...
- [x] **H6. DateTime cursor values formatted as ATOM strings, bound as DQL datetime params** — `Paginator.php:316-322`
...
- [x] **H7. Order-by regex matches empty `clause` for direction-less columns** — `Paginator.php:280`
...
- [x] **H8. `ENABLE_CURSOR_PAGINATION=false` in `PostgresBankRepository::getSearchQueryBuilder`** — `PostgresBankRepository.php:53-57`
...
- [x] **H9. Cursor unsigned + zip-bomb vector** — `PaginatorCursorFactory.php:17-37`
...
- [x] **M1. `getPageCount()` zero-mode + division-by-zero** — `Paginator.php:117`
...
- [x] **M2. `isSingleFirstPageQuery` overrides count incorrectly when filters/cursor narrow set** — `Paginator.php:96-99`
  Re-evaluated after H1: with the cursor predicate stripped from the count baseline, the heuristic only triggers on page 1 with no cursor narrowing, where `count(results)` legitimately equals the post-filter total. No fix needed; folded into H1.

- [x] **M3. Empty result resets `firstItem`/`lastItem`** — `Paginator.php:73-83, 96`
...
- [x] **M4. `#[Required]` setter injection on abstract repo** — `AbstractSearchRepository.php:18-22`
...
- [x] **M5. `(int) $page` cast accepts non-numeric strings as `0`** — `AbstractSearchRepository.php:32-38`
...
- [x] **M6. Parameter-name collision when alias contains underscores** — `Paginator.php:184`
...
- [x] **M7. Composite-key path disables fetch-join walker** — `AbstractSearchRepository.php:84`
  Override now gated on `!$this->hasNonRootSelect($queryBuilder, $alias)`: composite-PK QBs that select joined entities keep `fetchJoinCollection=true` (avoids cartesian count), while pure root-only selects still get the WHERE-IN bypass. PHPStan + lint clean.

- [x] **M8. Double serialize → json_decode → JsonResponse round-trip** — `BanksGetController.php:34-37`
...
- [x] **M9. `reference.php`: translator `enabled` doc default flipped `true → false`** — `api/config/reference.php:302`
  Already reverted in commit `8bbb215`. Verified `Default: true` on disk.

## Patch — LOW

- [x] **L1. `AbstractRepository` LSP drift on `createQueryBuilder` return type** — `AbstractRepository.php:21-26`
...
- [x] **L2. `Paginator` not `final` despite private state** — `Paginator.php:21`
  Subclassing risks breaking invariants (cached `$iterator`, set-once `$hasMorePages`).
  *Fix:* mark `final`.

- [x] **L3. `createFromString` swallows malformed cursor → empty cursor** — `PaginatorCursorFactory.php:18-37`
...
- [x] **D1. Order-by source becoming user-controlled → SQLi via `sprintf` of raw `$orderBy`** — `Paginator.php:184`
  ...
  *Applied:* `ORDER_BY_IDENTIFIER_PATTERN` allow-list enforced in `getOrderByColumns()`; non-matching identifiers throw `InvalidArgumentException` before reaching `sprintf`.
...
- `make php.lint` — clean for files in this review. Two pre-existing `CouplingBetweenObjects` violations remain on `BankPostController` and `BankPutController`, untouched by this review.
- Unit/Behat coverage for new pagination boundaries (cursor tampering, exact-multiple page sizes, timezone-divergent datetimes, malformed cursor → 400) **not yet written** — flagged for follow-up.
...
3. Tests: add unit coverage for `Paginator` and `PaginatorCursorFactory` boundary cases (empty page, exact-multiple 
   page size, malformed cursor, oversized cursor, cursor with timezone-divergent datetime).
