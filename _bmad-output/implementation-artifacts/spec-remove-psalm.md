...
- Un guardarraíl impide que psalm vuelva por la puerta de atrás.
...
- [x] `api/tests/Unit/Shared/Architecture/IsolatedToolingGateTest.php` -- guardarraíl: psalm **prohibido** (sin árbol propio), behat **aislado** (presente en su árbol, ausente del de la app)
...
**Acceptance Criteria:**
- Dado el repo, cuando se ejecuta `git grep -i psalm`, entonces solo quedan menciones históricas deliberadas (la regla en `api/CLAUDE.md`, el guardarraíl y su justificación).
...
- `make php.unit c='--filter IsolatedToolingGateTest'` -- 6/6 ✅
...
- `make php.unit` -- OOM preexistente, aislado con experimento controlado (mismo fallo con phpunit 13.1.14; víctima variable por corrida)
