...
- **(Shared/Images — modelado) Si un `Image` es mutable o inmutable no está decidido, y la URL de variantes necesita la respuesta.** `/{imageId}/{hash}/{variant}` lleva id y digest: con un agregado inmutable el digest es redundante con el id; con uno reemplazable es una decisión de modelado no declarada, y roza la invariante 2 (dos subidas iguales producen dos imágenes distintas con el mismo hash). Se decide cuando el cacheo inmutable de variantes tenga consumidor real. **Verificado contra el árbol el 2026-08-31 (retro de la épica Shared/Images): sigue abierto, y su disparador no ha llegado — no hay variantes ni URL de variante, la rebanada entregada es canonical-only.** Lo que sí cambió bajo esta bala: `Image` es hoy una entidad Doctrine persistida (`api/src/Shared/Images/Domain/Entity/Image.php`, no el `final readonly` del pase original), el módulo no reemplaza nunca una representación, y `max-age` quedó en 3600 s y no en un año precisamente porque la inmutabilidad del IDENTIFICADOR no implica cacheabilidad indefinida de los BYTES. La decisión pendiente es la misma; el terreno sobre el que se toma, no.
