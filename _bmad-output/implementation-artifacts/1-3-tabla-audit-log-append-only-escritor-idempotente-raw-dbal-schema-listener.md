...
**Then** escribe `$entry->level->value` (`'activity'|'security'`) y `$entry->actor->type->value` (`'anonymous'|'system'|'api_key'|'user'`) como **strings planos** en columnas `VARCHAR`; la validez del valor la garantiza el **tipo enum en PHP** (un caso ilegal es inconstruible), no un tipo `ENUM` de Postgres ni un `CHECK` (no existe precedente de ninguno en el repo — `BankAccountStatus`→`TEXT`, `Currency`→`VARCHAR`). **Decidido (D2):** `level`/`actor_type` se persisten como `VARCHAR` guardando `$enum->value`; **no** se usa `EnumType`/`EnumTypeValidator` (es una constraint de Symfony Validator, no un Doctrine `Type` — error de categoría del epic) ni enum nativo Postgres. Ver **Decisión D-1.3.d**. La redacción del `epics.md` (Story 1.3 AC / Additional Requirements) que menciona reutilizar `EnumType`/`EnumTypeValidator` se corrige aparte.
...
**AC11 — `audit_log` queda registrada como PII en cuanto nace la tabla.**
**Given** que `audit_log` contendrá PII (`actor_id`, `ip`, `user_agent`) en cuanto haya captura,
**When** se crea la tabla en esta historia,
**Then** se registra explícitamente que **`audit_log` ES PII** en `PRODUCTION_SECURITY_CHECKLIST.md` y en [`docs/rules/security.md`](../../docs/rules/security.md), nombrando las columnas PII (`actor_id`, `ip`, `user_agent`) y la **política prevista** (retención por nivel + pseudonimización/redacción GDPR);
**And** el registro deja claro que la **implementación** de poda/erasure llega en **Epic 3** — aquí solo se adelanta la **documentación "esto contiene PII + política"**, porque NFR3 y el **ADR-D4** exigen que el registro documental acompañe a la introducción de la tabla y no puede diferirse a Epic 3 (dejaría una tabla PII viva sin política documentada).
...
- [x] **T1 — `AuditLogSchemaListener`** (AC1, AC2, AC8, AC10) → `api/src/Shared/Audit/Infrastructure/Persistence/AuditLogSchemaListener.php`
  ...
  - [x] Columnas (usar constantes `Doctrine\DBAL\Types\Types`):
    ...
    - `correlation_id` → `Types::GUID` (NOT NULL) — **nota de dependencia (D4):** la no-nulabilidad la garantiza el fallback `Uuid::generate()` del adaptador de **1.4** (decisión confirmada); `correlation_id` nunca es null, así que la columna se queda `NOT NULL` sin riesgo.
    ...
    - `occurred_on` → `Types::DATETIMETZ_IMMUTABLE` (resuelve a `TIMESTAMP(6) WITH TIME ZONE` vía el `DateTimeTzMicrosType` registrado globalmente en `doctrine.yaml`; el escritor emite microsegundos `Y-m-d H:i:s.uP`, así que la columna debe ser `(6)` para no truncar)
...
- **No** añadir un enum nativo Postgres / `CHECK` para `level`/`actor_type` (D-1.3.d, **decisión D2 cerrada: `VARCHAR`**) ni un custom Doctrine type para `ip` (D-1.3.e, **decisión D3 cerrada: `VARCHAR(45)`**).
...
- **Seguridad / `audit_log` es PII:** esta historia **no** persiste PII todavía (no hay captura — eso es Epic 2/1.5), pero la tabla la contendrá (`actor_id`, `ip`, `user_agent`). La **implementación** de la política de retención + pseudonimización (poda/erasure) es **Epic 3**. Lo que **sí** entra en 1.3 (AC11 / T8) es el **registro documental "`audit_log` es PII + política prevista"** en `PRODUCTION_SECURITY_CHECKLIST.md`/[`docs/rules/security.md`](../../docs/rules/security.md): NFR3 y el ADR-D4 exigen que ese registro acompañe a la introducción de la tabla y no puede diferirse a Epic 3 (dejaría una tabla PII viva sin política documentada). Además, `ip`/`user_agent`/`metadata` son **datos no confiables (tainted)** controlados por el cliente: escaparlos al renderizar (UI admin Epic 4), nunca tratarlos como confiables. El escritor parametriza todo (`:placeholder`), sin interpolación de strings en SQL (checklist de inyección de `CLAUDE.md`).
...
PRODUCTION_SECURITY_CHECKLIST.md                                                     (MOD — registrar audit_log como PII: actor_id/ip/user_agent + política prevista, impl en Epic 3)
docs/rules/security.md                                                               (MOD — registrar audit_log como tabla PII con la misma nota)
```
...
- **Listener (unit, puro):** `extends TestCase`, sin kernel ni BD. Mirror exacto de [`EventStoreSchemaListenerTest`](../../api/tests/Unit/Shared/Event/Infrastructure/Persistence/EventStoreSchemaListenerTest.php): construir `new Schema()`, invocar el listener, asertar tabla/columnas/índices + idempotencia del guard. Añadir las aserciones de nulabilidad (`getNotnull()`) para fijar el contrato `actor_id` null / `correlation_id` not-null / `metadata` not-null, **y las de longitud (`getLength()`) por columna** (`action`/`resource_type` = 100, `ip` = 45, `user_agent` = 512, `level`/`actor_type` = 16) para que un drift de `length` rompa el unit en vez de filtrarse a una revisión manual de la migración (W7).
...
## Decisiones cerradas y preguntas pendientes

1. **[D2 — cerrada] `level`/`actor_type` como `VARCHAR` (D-1.3.d).** El AC del epic dice "reutilizan `EnumType`/`EnumTypeValidator`", pero `EnumType` es una *constraint de Symfony Validator*, no un mapeo Doctrine, y no aplica a una tabla raw sin entidad (error de categoría del epic). **Resuelto:** se persisten como `VARCHAR` guardando `$enum->value`; **no** se usa `EnumType`/`EnumTypeValidator` ni `CREATE TYPE … AS ENUM`/`CHECK` (patrón de la casa: cero enums nativos en el repo). La redacción del `epics.md` (Story 1.3 AC / Additional Requirements) que menciona reutilizar `EnumType`/`EnumTypeValidator` se corrige aparte.
2. **[D3 — cerrada] `ip` como `VARCHAR(45)` (D-1.3.e).** `Types::INET` no existe en DBAL y no hay ninguna columna `inet` en el repo; ninguna consulta de Epic 4 usa operadores de subred/CIDR — solo se almacena evidencia. **Resuelto:** `ip` se modela como `VARCHAR(45)`. El esbozo de esquema del ADR (que dice `ip inet`) se actualiza aparte a `varchar(45)`.
3. **[pendiente] Documentación de arquitectura.** ¿Documentamos `audit_log` en `docs/architecture-api.md` ya en 1.3, o lo diferimos a 1.4/1.5 cuando exista el flujo end-to-end (escritor cableado al transporte)? **Recomendación: diferir a 1.4/1.5.**
