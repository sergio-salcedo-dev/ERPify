...
- **Iteración 1 — pasada adversarial sobre el spec, tres capas en paralelo.** Ninguna falló; entre las tres, ~30 hallazgos distintos. Lo que refutó, medido y verificado a mano después:
  ...
  6. **La PR no cumplía el gate que documenta.** El registro de la pasada vivía en el cuerpo de la PR, que por construcción no puede preceder a la PR. Ahora vive en este artefacto. El criterio a posteriori que se escribió aquí —fecha del commit en la rama remota contra `createdAt`— **no es comprobable**: `%cI` lo reescribe cualquier rebase y `%aI` es asignable (#799). El gate que sí lo comprueba lo hace en el instante de crear la PR, no después.
