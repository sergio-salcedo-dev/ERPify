...
- **Revisión de diseño de Sergio (posterior al pase adversarial, previa al merge)** — veredicto KEEP sobre la decisión y sobre el endurecimiento del gate, con un punto accionable: el lector de rutas PHP debía **anunciarse como heurística de confinamiento**, no dejar que su rojo se lea como prueba semántica. Enmendado en los tres sitios donde alguien lo leería — el mensaje del propio fallo, el docblock de `PublicAccessPrefixBoundary` y la cabecera del registro — diciendo explícitamente qué NO prueba: un rojo significa «no puedo establecer el confinamiento», nunca «esta ruta es alcanzable», y su verde significa «ningún fichero PHP de rutas NOMBRA el prefijo», no «ninguno registra una ruta bajo él» (una ruta compuesta en tiempo de ejecución le es invisible, y cerrar ese residuo pide un loader real, no una regex más ancha). Medido en tres direcciones tras el cambio: ruta `.php` viva → rojo; mención sólo en un **comentario** → rojo, que es exactamente la imprecisión que el mensaje ahora declara; fichero `.php` sin mención → verde. Estado malo evitado: un control conservador leído como si fuera un parser de Symfony, que es cómo un límite conocido se convierte en una garantía que nadie prometió.
