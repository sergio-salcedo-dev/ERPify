...
- **La convención de commit dejó de nombrar la historia, y con ella se apagó medio `bmad.status.audit`.** Su *check B* —«historia en `review`/`in-progress` cuyo tag aparece en un commit que cambió algo que no es documentación»— necesita el tag en el asunto (`feat(iam): … (U-4) (#508)`). Medido sobre los 596 asuntos de `main`: **`G-2` y `G-5` no aparecen en ninguno**, y `G-4a`, `G-1a`, `G-1b`, `G-1c`, `G-3a`, `G-3b` **solo aparecen en commits `docs(…)` y `chore(bmad)`** — exactamente los que el check descarta, y exactamente el patrón que su cabecera ya declara (*«a story is named by the commit that writes its context, by the chore that closes its siblings … none of which ship it»*). Luego G-2 mergeó con marcador `in-progress` y el audit dijo *«matches origin/main»* **sin fallar**: no tenía un solo candidato que examinar. El control no se rompió; se quedó sin entrada.
- **Un `#[AsSchedule]` puede compilar, registrarse y desplegarse muerto.** La review de G-3b encontró que el schedule diario **no podía dispararse jamás**: sin `->stateful()`, `from` vive en memoria del proceso, y los consumidores mueren cada hora con `--time-limit=3600`, así que el próximo tick queda siempre ~23 h más allá de un proceso que vive 1 h. Era **preexistente en forma** para los tres schedules, y G-3b los arregló **los tres** (`identity_maintenance`, `audit_maintenance`, `maintenance` llevan hoy `->stateful($this->checkpointState)`, cada uno con el porqué en su docblock). El gate nuevo prueba que el *nombre* está cableado y **declara explícitamente que no prueba entrega**: el hueco entre «cableado» y «tickea» sigue cubierto por lectura, no por control.
...
> **Corrección durante la propia retro, y es su mejor evidencia — igual que en users-admin.** Esta lista nació con **seis** items y **dos eran falsos**, detectados al ir a implementarlos:
> **(a)** *«`->stateful()` para los otros dos schedules»* — los **tres** ya lo llevan; G-3b los arregló todos y yo escribí el item leyendo el texto de la tarea (*«decidir si se arregla aquí para los tres»*) sin medir el resultado.
> **(b)** *«el audit no vio esta épica con 8/8 en `done`»* — **la vio**: su *check A* es lo que abrió la sesión de esta retro. Y su *check B* no «casa contra asuntos de commit» a secas: el asunto selecciona candidatos y **la lista de ficheros decide**, endurecido precisamente por este patrón.
> Las dos son la lección de la retro anterior cometida por quien la estaba citando: **verifica contra `main` antes de tratar un defer como abierto**, y **mide la consecuencia, no el precondicionante**. Se dejan escritas en vez de borrarse en silencio.

### Resoluble ya

1. **Afirmar la siembra antes de afirmar la ausencia** — convertir en costumbre revisable lo que arregló los dos tests vacuos (G-1b, G-5): todo testigo que asierta «no queda ninguna fila» afirma primero que su `INSERT` afectó a N filas. Viñeta en `docs/rules/testing.md`. — *API / proceso*
...
### Decisión propia — las tres tomadas por Sergio (2026-08-04) y aplicadas en esta misma PR

3. **El pase adversarial pasa a ser requisito para *abrir* el PR**, no para llegar a `done`. La redacción anterior se midió permitiendo exactamente lo que existía para impedir: bajo *«registrado antes de `done`»*, #616 mergeó y su pase llegó en #618, y el patrón se repitió en #620 — con la entrega en `main` durante la ventana, y el pase posterior encontrando un defecto **GRAVE**. Un pase cuyos hallazgos solo pueden aterrizar en un *segundo* PR es una review, no una puerta. *Consecuencia declarada:* un PR que necesita ojos antes de estar listo es un **draft**, y el pase es lo que lo saca de draft. `CLAUDE.md` → *Security review* → **Process**.
4. **Vuelve el tag de historia al asunto de los commits que llevan código** — `feat(shared): … (G-2) (#636)`. Es la única entrada del *check B* del audit: sin él el control no se debilita, **enmudece**. Queda escrito en `CLAUDE.md` → *Commit messages* con la medición que lo motiva. *Descartado el hook de `commit-msg`:* exigiría el tag también donde no hay historia y es fricción en cada commit; la convención escrita se revisa en el propio PR, que es donde ya se mira.
5. **La promesa de D12 se ablanda: «cerrado» es cerrado por revisión, no por gate.** Ambos ADR —`event-store-and-projections.md` D12 y su gemelo `audit-activity-log.md`— dicen ahora que `git grep` es el único control y que no está cableado a ninguna puerta. El gate queda con su trigger explícito (*la primera propuesta de una segunda mutación sobre cualquiera de las dos tablas*), porque con un solo miembro por tabla no cumple la Regla de Tres. Prometer un control inexistente era el defecto que esta épica entera existía para eliminar.
