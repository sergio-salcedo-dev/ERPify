...
3. **El gate de proceso llegó tarde dos veces (#616→#618, y su repetición en #620).** Decidir qué lo mueve antes del merge: ¿el pase adversarial es requisito de *abrir* el PR, o basta con que su registro apunte al PR que lo aplica? Hoy la regla dice «registrado y declarado dónde», y eso admite el orden que se dio.
4. **La convención de commit dejó sin entrada al *check B* del audit.** Ninguno de los ocho commits que llevan el código nombra su historia; los únicos que la nombran son documentales, que es justo lo que el check descarta. Decidir: **recuperar el tag de historia en el asunto de los commits de código** (`feat(shared): … (G-2) (#636)`) y devolverle candidatos, o **aceptar que el check B es inerte** bajo la convención actual y decirlo en su cabecera, donde hoy se describe como un punto ciego parcial y en realidad es total para una épica entera.
5. **El «conjunto cerrado de mutaciones sancionadas» de D12 y su gemelo de `audit_log`.** Está diferido con trigger (*la primera propuesta de una segunda mutación*), pero el ADR sigue prometiendo «cerrado» mientras `git grep` es el único control. Decidir si la promesa se ablanda o el conjunto se cierra de verdad.
