...
- [x] **[Patch · aplicado] DRY del file-walk `api/src` en `ApiSourceFiles`** [`api/tests/Support/ApiSourceFiles.php`] — el barrido recursivo estaba duplicado entre `MarkerStatusMapContractTest` (autoload + reflexión) y `ErrorContractGateTest` (grep textual). Extraído a un único helper de test; cada gate mantiene su derivación encima. Verificado: PHPStan 0, ambos gates verdes (17/17), `php.quality` sin findings.
- [x] **[Resuelto · documentado] Contrato de precisión de `parseStrict`** [`docs/api-error-contract.md`] — añadida la línea que fija que `invalid-search-value` acepta solo ISO-8601 byte-canónico con offset UTC real (`Z`/`+00:00`) a precisión de segundos / 3 dígitos (millis JS) / 6 (micros); cualquier otra anchura fraccional o forma no canónica → 422, nunca coerción al instante más próximo. Antes solo vivía en el docstring de `FilterApplier`.
- [x] **[Resuelto · mantener, sin cambio] El gate dual-marker autocarga todo `api/src`** [`api/tests/Unit/Shared/Application/Problem/MarkerStatusMapContractTest.php`] — evaluado y conservado a propósito. El scan completo es **necesario para la exhaustividad**: un subtipo futuro que herede ≥2 markers sin nombrarlos textualmente sería invisible a cualquier pre-filtro por ruta/nombre (`RateLimited`/`NotFound`/`Conflict` no llevan "Exception"). El coste de autoload es sub-ms y, bajo PSR-1, los ficheros de definición no tienen efectos en load-time, así que el "fallo confuso" es teórico (una clase no autocargable rompería otros tests igual). Un guard try/catch protegería un caso que PSR-1 prohíbe → especulativo. Sin mitigación que merezca la pena.
