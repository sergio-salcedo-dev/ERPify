...
- [x] [Review][Patch] Docblock de `LoginController` — **corregido**: describe el default `_format: json` de las rutas API que hace que `json_login` atienda el POST → 400 por decode fallido (no "content negotiation before routing"). `api/src/Backoffice/Identity/Infrastructure/Http/LoginController.php`
- [x] [Review][Patch] Escenario Behat 4 (email en blanco) — **corregido**: ahora asevera `title == "Invalid credentials."`, pinneando la indistinguibilidad del caso malformado. `api/features/backoffice/identity/login.feature`
- [x] [Review][Defer] `InMemoryUserRepository::findById()` fake infiel — **NO aplicado por diseño**: hacerlo fiel exige exponer `User::id()` en el dominio, que hoy no tiene consumidor (el login usa `findByEmail`; `findById` no se usa en el flujo de auth). Añadirlo ahora sería especulativo (YAGNI); E3 introducirá el accessor + un consumidor real de `findById` (token→UUID) — revisar entonces. `api/tests/Unit/Backoffice/Identity/Application/InMemoryUserRepository.php`
- [x] [Review][Patch] Higiene de registro — **hecho**: podados de `deferred-work.md` los 2 bullets de AF-1.1 que esta PR resuelve (`Validator::ensure` en el alta; patrón de traducción de errores del `UserProvider`).
