...
**Given** una request real `GET /api/v1/backoffice/banks/{bankId}/accounts` que lista las cuentas de un banco existente, procesada de extremo a extremo (incluido el consumo del transporte `audit` por un worker),
**When** se procesa,
...
**AC5 — Idempotencia: la reentrega at-least-once deja una sola fila.**
**Given** la entrega at-least-once del `messenger_worker` sobre el transporte `audit`,
**When** el mismo `RecordAuditEntry` se reprocesa (redelivery),
**Then** sigue habiendo **una sola** fila para ese acceso: el `INSERT … ON CONFLICT (id) DO NOTHING` de 1.3 lo hace idempotente por el `id` UUIDv7 acuñado en `AuditLogEntry::create()` (1.2/1.4); un redelivery es un no-op, sin duplicar la traza (FR4, NFR7) — Behat.
  ...
  - [ ] **Escenario 1 — una fila forense correcta (AC4):**
    ...
    - Asertar la fila vía `SqlQueryContext`. Una sola consulta por columnas explícitas (sin `SELECT *`, NFR9), con `correlation_id` filtrado al de la request:
      ...
      And the SQL result as JSON should be:
        ...
        """
      ```
      Ajustar el literal de `metadata` a lo que Postgres devuelva para un `[]` JSONB (`'{}'` o `'[]'`; comprobar contra la BD real — el `DbalAuditLogWriter` de 1.3 escribe `json_encode([])` = `'[]'`, que Postgres normaliza; usar la forma que devuelva `fetchAllAssociative`). **Aserción anti-PII explícita** (FR12): `And the SQL result as JSON should be:` no contiene IBAN ni campos de cuenta — el `metadata` vacío lo garantiza; opcionalmente añadir un assert `the SQL result ... should not contain "DE89370400440532013000"` si existe un step de no-contención, o dejarlo cubierto por el match exacto del JSON.
  - [ ] **Escenario 2 — idempotencia por reentrega (AC5):**
    - Repetir la request + consumo del escenario 1 para crear **una** fila, luego **reinyectar el mismo mensaje** y reconsumir, asertando que sigue habiendo **una** fila. Dos enfoques (elegir el que case con los steps reales de 1.4/del `MessengerConsumerContext`):
      - (a) Preferido si el handler es re-invocable con el mismo `RecordAuditEntry`: consumir el transporte `audit` **dos veces** el mismo mensaje no es trivial con `in-memory` (la cola se drena al consumir). Por eso, modelar el redelivery a **nivel de escritor**: tras la primera fila, ejecutar un `INSERT … ON CONFLICT (id) DO NOTHING` con **el mismo `id`** ya presente y asertar 0 filas nuevas — pero el `id` no es observable desde Behat. **Descartar (a).**
      - (b) **Recomendado:** asertar la invariante observable de extremo a extremo: emitir **la misma request dos veces** produce **dos** filas (dos accesos distintos → dos `id` distintos, AC6 de 1.3 — esto NO es lo que pide AC5). Para AC5 (mismo mensaje, mismo `id`) la garantía la da el `ON CONFLICT` ya **probado en integración en 1.3** (`AuditLogWriterIdempotencyTest`); en Behat, la cobertura end-to-end correcta del redelivery es **consumir el mismo mensaje encolado más de una vez**. Si el `MessengerConsumerContext`/transporte de 1.4 permite reencolar o reconsumir el mismo envelope (p. ej. un step "the audit message is redelivered"), usarlo y asertar 1 fila. **Si no existe tal step**, dejar AC5 cubierto por el test de integración de 1.3 y documentarlo en la spec como tal, evitando un escenario Behat frágil que afirme idempotencia sin poder forzar el redelivery real. **Elevar esto como Pregunta 2** (qué mecanismo de redelivery expone 1.4 a Behat).
    - Hasta resolver Pregunta 2, escribir el escenario de idempotencia **solo** si 1.4 provee un step de reentrega; en su defecto, anotarlo con `@todo`/omitirlo y apoyarse en el `AuditLogWriterIdempotencyTest` (AC5 de 1.3) para la garantía por-PK, citándolo en *Completion Notes*.
...
- **Idempotencia (AC5):** la garantía por-PK (`ON CONFLICT (id) DO NOTHING`) está **probada en integración en 1.3** (`AuditLogWriterIdempotencyTest`). En Behat solo es ejercitable de extremo a extremo si 1.4 expone un mecanismo de **redelivery del mismo envelope** (mismo `id`); si no lo expone, **no** forzar un escenario Behat frágil — apoyarse en el test de 1.3 y dejarlo dicho en *Completion Notes*. Ver Pregunta 2.
...
2. **Mecanismo de redelivery para el escenario de idempotencia Behat (AC5).** ¿Expone 1.4 / el `MessengerConsumerContext` una forma de **reentregar el mismo envelope** (mismo `id`) sobre el transporte `audit` (p. ej. un step "the audit message is redelivered" o reconsumir sin drenar)? Si **sí**, escribo el escenario de idempotencia end-to-end (consumir 2× el mismo mensaje → 1 fila). Si **no**, AC5 queda cubierto por el test de integración por-PK de 1.3 (`AuditLogWriterIdempotencyTest`) y **no** escribo un escenario Behat frágil que afirme idempotencia sin poder forzar el redelivery real. **Recomendación:** apoyarse en el test de 1.3 para la garantía por-PK; añadir el escenario Behat de redelivery **solo** si 1.4 da el step. (No bajar el transporte a `sync` en test para hacerlo "fácil" — falsearía el modelo async, D-1.5.e.)
3. **Scope del commit y de `docs/architecture-api.md`.** ¿Confirmas scope `backoffice` para el commit (el cambio de comportamiento vive ahí), o prefieres `shared` para alinear con los commits 1.1–1.4 del epic? Y: ¿la línea del eje `audit_log`/transporte `audit` en `docs/architecture-api.md` la trajo 1.4, o la añado en 1.5? **Recomendación:** scope `backoffice`; y que `architecture-api.md` lo documente 1.4 (cuando cableó el flujo) — si 1.4 lo difirió, lo añado en 1.5 como mejora-en-scope nombrada.
