...
_Code review adversarial (Blind Hunter · Edge Case Hunter · Acceptance Auditor) — 2026-06-24. Las 8 ACs se verificaron satisfechas; los hallazgos son aislamiento de test e higiene, no incumplimientos de AC._

- [x] [Review][Decision] Bullet de `architecture-api.md` (eje `audit_log`) añadido en 1.5 pese a estar abierto en la Pregunta 2 — **RESUELTO (Sergio, 2026-06-24): mantener en 1.5** como mejora-en-scope nombrada (documenta el eje al estrenarse el primer actor real); **commit scope `backoffice`**. El bullet ya es correcto; se reconcilia el matiz de las Completion Notes ("añadido en 1.5, confirmado", no "no añadido unilateralmente").
- [x] [Review][Decision] `docs/rules/cqrs-naming.md`: colapso de 5→4 categorías (retira la categoría del subscriber de auditoría) — **RESUELTO (Sergio, 2026-06-24): opción 2 → conservar la categoría con nota "sin instancias vivas", NO colapsar 5→4.** La taxonomía describe mecanismos arquitectónicos permitidos (frontera audit-subscriber ≠ domain-event-subscriber), no la cardinalidad actual del código; cardinalidad 0 ≠ categoría inexistente. Minimiza churn y conflicto con la PR #325. Convertido en patch (ver abajo).
- [x] [Review][Patch] (APLICADO+verificado 2026-06-24) Revertir el colapso 5→4 en `docs/rules/cqrs-naming.md` y conservar la categoría de audit-subscriber con nota "sin instancias vivas" `[docs/rules/cqrs-naming.md]` — Restaurar la estructura de 5 categorías (deshacer el renumerado), reemplazar el ejemplo muerto `RecordAuditLogOnBankAccountsViewed` por una nota: la categoría se conserva porque representa una frontera arquitectónica distinta del domain-event-subscriber, actualmente sin implementación viva (un futuro `RecordAuditLogOnInvoiceViewed` cae aquí: `Infrastructure/Audit/<Effect>On<X>`). Diff mínimo para mergear bien con #325.
- [x] [Review][Patch] (APLICADO+verificado 2026-06-24) El transporte `audit` in-memory no se drena entre escenarios Behat → flake order-dependent `[api/tests/Behat/Context/OutboxContext.php:55]` — `INSPECTABLE_QUEUES = ['async','failed']` (el `#[BeforeScenario] reset()` no toca `audit`). `BankAccountSearcher` ahora encola un `RecordAuditEntry` en `audit` en cada lectura con éxito; `search.feature` dispara ~7 lecturas y nunca consume/drena la cola. `audit.feature` consume "1 message" del `audit` y hoy pasa **sólo** porque corre antes que `search.feature`; bajo reordenación/paralelización/segundo escenario consumiría un envelope viejo (correlation_id/resource erróneos) → falla el match exacto de JSON. Fix recomendado: drenar la cola `audit` por escenario **sin** meterla en `INSPECTABLE_QUEUES` (que es el vocabulario de conteo del outbox de domain-events) — p. ej. una `RESETTABLE_QUEUES = ['async','failed','audit']` usada sólo por `reset()`.
- [x] [Review][Patch] (APLICADO+verificado 2026-06-24) El test de página vacía sólo asserta `action`, no el recurso `[api/tests/Unit/Backoffice/BankAccount/Application/BankAccountSearcherTest.php]` — `testRecordsTheAccessEvenForAnEmptyPage` asserta `records[0]['action']` únicamente, mientras que el camino feliz asserta action+level+resource+metadata. Una regresión en el camino de página vacía (recurso erróneo/nulo) quedaría verde. Reforzar asertando `resource_type='Bank'` + `resource_id=$bankId` también en la página vacía.
- [x] [Review][Defer] Las escrituras raw-DBAL a `audit_log` no marcan el `FixturesChangeTracker` (restore por feature puede saltarse) `[api/tests/Behat/Doctrine/FixturesWriteListener.php:16]` — deferred, pre-existing — El listener es `onFlush`-only; un request de sólo-lectura que produce una fila `audit_log` (vía el insert raw-DBAL del worker) no dispara `onFlush` → el tracker puede quedar limpio → el restore del siguiente escenario se salta → la fila `audit_log` se filtra hacia delante. Benigno HOY (`audit.feature` es el único lector+escritor de `audit_log`, un solo escenario), pero latente si aterriza un 2º escenario/feature que lea `audit_log`. Limitación del harness (misma propiedad que `event_store`/`bank_count`, enmascarada por operaciones que sí hacen flush ORM).
