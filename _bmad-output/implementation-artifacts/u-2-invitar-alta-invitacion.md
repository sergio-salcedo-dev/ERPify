...
Revisión adversarial (Blind Hunter · Edge Case Hunter · Acceptance Auditor, Opus 4.8) triada contra el código real del worktree `33fa27d6`. AC1–AC13 verificados como implementados; 1 decisión, 6 patches, 4 diferidos, 6 descartados como ruido. Sin hallazgos High.

**Aplicado y verificado (2026-07-18).** La decisión y los 6 patches quedan aplicados en la rama (sin commitear aún). `Role` reubicado a `Erpify\Shared\Access\Domain\Role` (52 ficheros: enum + test como renames, 44 referencias, allowlist + deptrac limpios de los 6 seams `Role`). Gates verdes: `php.stan` (1090, OK), `php.deptrac` (0 violaciones), `php.lint.bounded-context`, `php.lint.error-contract`, `php.quality` (sweep completo), 435 unit + 51 functional (áreas tocadas), Behat `invitation_create` 6/6, `pwa.quality`, 25 vitest (incl. el nuevo test de página P6). El único fallo de la suite unit completa es el flake preexistente `BankLogoMultipartFunctionalTest` («Premature end of PHP process»), ajeno a este cambio.

**Decisión (requiere tu input):**

- [x] [Review][Decision] **RESUELTO → reubicar ahora.** `Role` movido a `Erpify\Shared\Access\Domain\Role` (capability Shared `Access/`, espeja el PWA `context/shared/access/domain/Role.ts`); las 6 entradas `Role` de `.bounded-context-allowlist` y los 6 `skip_violations` de `deptrac.yaml` eliminados (referencias ahora legales vía `Shared.Domain`). Reubicar `Role` a un kernel compartido de acceso vs. mantener la exención de vocabulario por-fichero — `Role` vive en `Iam/Identity/Domain/Enum/Role.php` pero se consume como vocabulario RBAC compartido por `Iam/Invitation` (CLI + HTTP) y `Organization/Membership`; cada consumidor paga una entrada en `.bounded-context-allowlist` **y** en `deptrac.yaml skip_violations` (este PR añade 2). Mover `Role` a `Erpify\Shared\…` (siempre importable) borraría todas esas exenciones de golpe. El status quo es defendible por precedente (misma exención ya concedida a `CreateInvitationCommand`). Es una llamada de frontera de contexto → decisión tuya (no la resuelvo unilateralmente).

**Patches (fix inequívoco):**

- [x] [Review][Patch] [Medium] Falta `#[Assert\Length(max: 255)]` en `InviteUserRequest::$email` — un cliente que POSTee directo a la API un email válido de >255 chars pasa DTO+entidad (egulias STRICT trata la longitud como *warning*, no error) y golpea `identity_user.email VARCHAR(255)` → SQLSTATE 22001 → **500**, rompiendo la promesa de AC5 («422 antes del dominio») para el contrato de la API. El zod del cliente ya capa a 255 y los DTO hermanos (`AcceptInvitationRequest`, `ResetPasswordRequest`) emparejan `Assert\Length`. Añadir la constraint al DTO (e idealmente a la entidad `User`). [api/src/Iam/Invitation/Infrastructure/Http/InviteUserRequest.php:25]
- [x] [Review][Patch] [Low] Submit pre-hidratación filtra el email del invitado (PII) a la URL/historial/logs/Referer — el `<form>` no declara `method`/`action` y el botón solo está `disabled={isSubmitting}` (no deshabilitado hasta hidratar); `data-hydrated` solo sirve para que la automatización espere. Un usuario que envíe en la ventana de hidratación dispara `GET /backoffice/users/invite?email=<invitado>&roles=…`. Fix: derivar `disabled` (y el `tabIndex` de Cancel) también de un estado hidratado, o renderizar el form client-only. [pwa/src/app/backoffice/users/_components/InviteUserForm.tsx:1145]
- [x] [Review][Patch] [Low] El test «double submit fires one invitation» nunca hace un segundo submit — envía una vez y asserta botón `disabled` + `inviteRun` llamado 1 vez (trivialmente cierto). Fix: disparar un 2.º submit con la promesa pendiente y assertar que `inviteRun` sigue en 1 (o renombrar el test a lo que realmente comprueba). [pwa/tests/app/backoffice/users/inviteUserForm.test.tsx:80]
- [x] [Review][Patch] [Low] El `Scenario Outline` 422 de AC5 no fija «0 identidades creadas» — asserta 422/type/0-eventos/0-emails pero omite el `SELECT … FROM identity_user` + `0 records` que el AC exige (el escenario 403 hermano sí lo fija). Fix: añadir la aserción SQL de conteo al outline. [api/features/backoffice/identity/invitation_create.feature:523]
- [x] [Review][Patch] [Low] Rama muerta `?? field` en `baseField` — `tsconfig` no tiene `noUncheckedIndexedAccess`, así que `split(...)[0]` tipa `string` y el fallback es inalcanzable. Fix: `return field.split(/[.[]/)[0];`. [pwa/src/app/backoffice/users/_components/InviteUserForm.tsx:34]
- [x] [Review][Patch] [Low] El fallback `permission-denied` del `<Can>` en la página de invitación no tiene test (solo el 403 de la API y el happy-path están cubiertos). Fix: un render test que asserte el `EmptyState variant="permission-denied"` cuando falta el permiso. [pwa/src/app/backoffice/users/invite/page.tsx:11]

**Diferidos (real, pero pre-existente / sistémico / fuera de alcance):**

- [x] [Review][Defer] [sistémico] Fallo de red / no-`HttpError` en submit → no-op silencioso — `FetchHttpClient.request()` hace `await fetch()` sin try/catch, así que un fallo offline/DNS/CORS lanza un `TypeError` crudo que `onSubmit` re-lanza (`if (!(err instanceof HttpError)) throw err;`) → rejection no capturada, el spinner para y no se muestra error. Idéntico a `BankForm.tsx:141` — patrón heredado del repo; el fix correcto es central (envolver el fallo de transporte en `FetchHttpClient`), afecta a todos los forms de backoffice. [pwa/src/context/shared/http-client/infrastructure/FetchHttpClient.ts:111] — deferred, pre-existing
- [x] [Review][Defer] [Low·seguridad] Sin política de roles del invitador — cualquier ADMIN puede acuñar ADMINs sin límite (`SendInvitation::invite(Role ...$roles)` sin filtrar; el form itera `ALL_ROLES` incl. ADMIN). Conscientemente fuera de alcance (spec «Fuera de alcance»), pero es una superficie de propagación de privilegio que merece issue + nota en la sección de seguridad del PR (y quizá una señal de auditoría distinta en grants de ADMIN). [api/src/Iam/Invitation/Application/SendInvitation.php:60] — deferred, out-of-scope
- [x] [Review][Defer] [Low] `InvitedIdentityUnavailable` sigue sin marcador → 500 en el nuevo path HTTP — AC7 marcó solo `UserAlreadyMember`; `OrganizationNotProvisioned` se dejó 500 a propósito (desviación #2, «estado de servidor imposible → 500 honesto»), pero `InvitedIdentityUnavailable::withoutId()` del mismo funnel no se nombró — mismo razonamiento (inalcanzable bajo sesión ADMIN gateada) pero sin discutir. Confirmar que la lógica «500 honesto» se extiende a él, o marcarlo. [api/src/Iam/Invitation/Application/SendInvitation.php:68] — deferred, pre-existing
- [x] [Review][Defer] [Low] El presupuesto de query exacto `26` acopla la feature de invitación a internals de auditoría/CDC/outbox — cualquier cambio no relacionado en el listener de auditoría/outbox/event-store mueve el número y hace fallar un escenario de *invitación* por una razón ajena. Patrón aceptado de la casa (`behat-query-budget-transaction-overhead`), pero un `<= N` repo-wide sería menos frágil. [api/features/backoffice/identity/invitation_create.feature:502] — deferred, accepted-pattern
