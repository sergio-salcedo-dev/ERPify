...
- Editar `g-5-ids-de-persona-fuera-del-event-store.md` (`status: done`), cuya línea 54 afirma que `Iam.Invitation => non-person` acierta. Default: **no tocarlo**, anotarlo en la PR.
...
E existe porque D dejó el paso negativo en `skipped` — Behat salta lo que sigue a un fallo, así que D por sí solo no probaba que la mitad negativa mordiera. Restauración siempre por copia de los bytes respaldados (`md5sum` verificado), nunca `git checkout --`.
