...
| 2026-07-02 | Opción B (consulta Winston+Amelia): puerto `findByEmail(Email)`, adapter = lookup puro sin validar. El repo deja de canonicalizar/validar; blank/garbage pasa a ser responsabilidad del `UserProvider` de AF-1.2 (`Email::from` en try/catch → `UserNotFoundException`). Cierra el landmine `findByEmail('')`=500 (`InvalidEmail` marker-less). |
