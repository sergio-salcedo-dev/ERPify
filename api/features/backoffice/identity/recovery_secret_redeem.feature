Feature: Redeem a recovery secret (the sole administrator's only lockout edge)
  ...
  # selectors exist and are worth draining.
  Background:
  ...
  Scenario: A valid secret clears a live lockout and establishes a session
    ...
    And there should have 1 records in SQL result
    # The audit row nothing else writes, and the one that is conditioned on the consumption having
    # PERSISTED rather than merely on the secret having verified. Removing the retirement reds this too.
  ...
  Scenario Outline: Every death case answers one byte-identical refusal
    # The four spellings differ in what is wrong with them and in nothing a caller can observe. A graded
