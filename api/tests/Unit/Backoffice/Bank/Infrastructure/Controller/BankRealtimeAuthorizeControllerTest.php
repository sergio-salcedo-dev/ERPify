 ...
 * decoded here to prove it authorizes exactly the two back-office bank topics (collection + per-bank
 * template) — the contract the PWA EventSource relies on.
 *
 * @internal
 */
#[CoversClass(BankRealtimeAuthorizeController::class)]
final class BankRealtimeAuthorizeControllerTest extends TestCase
{
    ...
    public function testCookieAuthorizesExactlyTheTwoBankTopicsAndPublishesNothing(): void
    {
    ...
    private function authorize(): Response
    {
        ...
        $controller = new BankRealtimeAuthorizeController($authorization);

        return $controller(Request::create('/banks/realtime/authorize'));
    }
