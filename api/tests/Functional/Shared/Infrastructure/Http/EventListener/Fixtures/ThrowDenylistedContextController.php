...
/**
 * Story 3.2 — test-only controller that throws a `DomainException` implementing the
 * `NotFound` marker with a `context()` carrying a mix of denylisted (`password`, `token`)
 * and safe (`safe_field`) keys, so the redaction denylist can be exercised end-to-end via
 * a real Symfony kernel (functional + Behat). The `'sensitive'` value lets the test
 * assert the substring is absent from the encoded body — defense-in-depth even if a key
 * escaped by some other path.
 *
