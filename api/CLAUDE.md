...
-   **A development tool may not constrain the app's Composer tree** unless it is needed to run the application or its test suite. Behat owns an isolated tree at `tools/behat/` for exactly this reason (it pins `symfony/*` to `^7` while the app is on 8), and psalm was removed rather than isolated once the same problem appeared. Any future tool that drags the app's dependencies backwards gets one of those two treatments. Gate: `IsolatedToolingGateTest` (runs in `make php.unit`).
