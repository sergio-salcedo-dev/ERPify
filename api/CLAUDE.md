...
-   **Clear `var/cache/psalm` before regenerating the psalm baseline.** A warm cache makes `make php.psalm.baseline` miss `findUnusedCode` findings, so `tools/psalm/psalm-baseline.xml` comes out incomplete — green locally, red on CI. Clear it, run `make php.psalm.baseline`, then re-run `make php.quality` (cache cleared again) and confirm `EXIT=0`; the baseline diff vs `origin/main` should be only the intended entries. Framework-instantiated DTOs/commands mapped by `#[MapRequestPayload]` / `#[MapQueryString]` are the usual `PossiblyUnusedMethod` source — baseline them or give them an explicit `new` caller.
