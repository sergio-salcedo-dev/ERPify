...
-   **A development tool may not constrain the app's Composer tree** unless it is needed to run the application or its test suite. Behat owns an isolated tree at `tools/behat/` for exactly this reason (it pins `symfony/*` to `^7` while the app is on 8), and psalm was removed rather than isolated once the same problem appeared. Any future tool that drags the app's dependencies backwards gets one of those two treatments. There is no automated gate — the symptom is silent (`composer update` simply declines to move a package), so diagnose it with `composer why-not <package> <version>`.
