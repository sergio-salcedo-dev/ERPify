...
- **Indexes (NFR4)** — only index-backed columns are exposed as sortable, so ordering never degrades to a filesort. Changing `sort` invalidates a keyset cursor (the cursor encodes the order columns); the paginator degrades gracefully to offset. A non-unique sort key needs a composite `(column, id)` index for total order; a unique column's single-column index already suffices — pinned by `SortFieldMapIndexContractTest`.
- **Keyset engine (forward-looking)** — the new `DoctrineSearchEngine` is the read-path query-shaper a search repository will delegate to: it supplies only its base query builder (`SELECT`/`FROM`/joins) + `searchFieldMap()`/`sortFieldMap()`, and the engine owns sort resolution, filtering, the keyset predicate, the +1 trick, and cursor encoding. **As of PR2 it is built and validated by direct tests but is NOT yet on the HTTP wire** — the legacy `Paginator` still serves these endpoints unchanged; repositories are wired to the engine in PR3. Do not route a new endpoint through the engine until then.
...
- Pagination caps live on `SearchCriteria::MAX_PAGE` / `SearchCriteria::MAX_LIMIT` (the domain single source of truth; `SearchQuery`'s `#[Assert]` and the Doctrine repositories reference them) and apply to every search endpoint.
