...
4. **Field maps** — the concrete repository defines `searchFieldMap(): SearchFieldMap` (the filter allow-list) and `sortFieldMap(): SortFieldMap` (the sort allow-list) and passes both into the engine. Return `new SearchFieldMap([])` / `new SortFieldMap([])` to expose nothing. Mark UUID columns with `requiresUuidValues: true` and name `operators: [FilterOperator::Eq, FilterOperator::In]` — the constructor rejects `contains` over a UUID, and `In` is not in the default so it has to be asked for so a malformed value is a 422 `invalid-search-value`, not a Postgres error. Mark timestamp columns with `requiresDateTimeValues: true` and list only the range operators (`gt`/`gte`/`lt`/`lte`): bounds parse as RFC 3339 datetimes, normalize to UTC, bind as a typed parameter. Filtering and sorting are independent allow-lists; keep only index-backed columns sortable (NFR4) and map a public name to an indexed expression when needed (e.g. `name` → `b.nameNormalized`). A `sort` outside the map is a 422 `unknown-sort-field`, never interpolated into DQL raw.
...
- **Operators** — `eq` · `in` · `contains` · `gt` · `gte` · `lt` · `lte`, strictly lowercase (the enum backing string IS the wire contract). Every operator is **allow-listed per field**, and `FieldMapping`'s default grants only `eq` and `contains` — so `in` and the range operators are **opt-in**: a field that does not name `In` in its `operators:` answers 422 `unsupported-search-operator` for a list, exactly as `filters[…][field]=name&…[operator]=gt` does on `banks`, where only `createdAt` / `updatedAt` accept ranges. `in` is deliberately absent from the fields carrying a person's address, an account holder's name and an IBAN.
...
- `ValidationFailedException` from the DTO is automatically mapped to a **422** `validation-failed` RFC 9457 Problem Details body (Symfony's `RequestPayloadValueResolver` wraps it in an `HttpException(422)`; `Erpify\Shared\ErrorContract\Infrastructure\Http\ProblemDetailsFactory` walks `getPrevious()` and re-emits it as 422 with the structured `violations` extension in place of Symfony's generic body). See [`../../docs/api-error-contract.md`](../../docs/api-error-contract.md).
...
The IBAN is always serialized **canonical**: upper-case, no spaces, no separators (`ES9121000418450200051332`) — the form it is persisted in. Masking is presentational on the client only; the backend never masks, and the value (classified PII) is **never logged**.

`iban` is **not** part of the generic GET `filters[]` vocabulary on `bank-accounts` — a query-string parameter can reach a proxy/CDN access log or an intermediary cache keyed on the URL regardless of any single deployment's redaction config (root `CLAUDE.md` → "Putting a value in a query string"). Filtering on it returns 422 `unknown-search-field`, the same shape as `status`/`currency`. Exact lookup by IBAN goes through a dedicated `POST /bank-accounts/iban-lookup` instead (`BankAccountIbanLookupController`, request DTO `LookupBankAccountByIbanQuery`): the value travels only in the body, canonicalized the same way (`Iban::canonicalize()`) before the equality match, a malformed IBAN is a 422 that never echoes the rejected value, and a miss is a 404 (`bank-account-not-found`) carrying no `iban` context key. `bank_account.iban` is `unique`, so this is a single-row lookup, never a paginated collection — see issue #426.
