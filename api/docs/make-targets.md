...
-   Drift gates (in `php.quality`): `php.lint.error-contract`; `php.lint.bounded-context` (bounded-context isolation — Level 1 cross-context `Domain`/`Application` import fails, Level 2 cross-context FK warns; seams in `api/.bounded-context-allowlist`).
