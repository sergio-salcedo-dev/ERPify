 ...
 * person's identifier into `messenger_messages` and, on a failure, into `failed`: Doctrine tables no erasure
 * path reaches, one unbounded and the other swept at 30 days, so a queued id would outlive the erasure the
 * application had already confirmed to its subject. `api/.persistent-transport-policy` would not catch it either — a
