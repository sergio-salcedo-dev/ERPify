 ...
 * controller edge where the other per-target throttles live. Two routes demand that proof — `POST
 * /me/password` and `POST /me/recovery-secret` — and they share ONE bucket.
 *
 * **Sharing it is a security decision, and the name says which question the budget answers rather than which
 * controller reached it first.** A wrong `currentPassword` on either route deliberately does not feed the
 ...
 * credential. Give the second route a bucket of its own and an attacker holding a stolen session gets twice
 * the guesses per window against the same password, with nothing else closing the gap — the per-IP
 ...
 * also spends the budget that guards their password change, and an attacker who drains it stalls both. That
 * is the residual the existing endpoint already accepted, unchanged in kind — nothing is destroyed, the
 * credential still works, and the window refills.
 *
 ...
 * exists" to someone who was only guessing. Both routes here are reachable only through an established
