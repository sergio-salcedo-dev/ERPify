...
/**
 * GDPR "right to erasure" for an identity subject: hard-deletes the user row (the module's PII — email and
 * credential hash) and every pending password-reset token, leaving a `GDPR_SUBJECT_ERASED` security audit
 * entry as the compliance record. Mirrors <comment>bank-account:gdpr:erase-subject</comment>; a console
 * command rather than an HTTP route so the trigger stays operator-gated.
 */
#[AsCommand(
    name: 'identity:gdpr:erase-subject',
    description: "Irreversibly erase an identity subject's data (GDPR right to erasure)",
)]
final class EraseIdentitySubjectCommand extends Command
{
    public function __construct(
        private readonly EraseIdentitySubject $eraser,
    ) {
        parent::__construct();
    }
    ...
    protected function configure(): void
    {
        $this
            ...
            ->setHelp(<<<'HELP'
                ...
                hash) together with every pending password-reset token, so no <comment>user_id</comment>
                linkage or recovery artefact outlives the subject. The erasure self-audits as a
                <comment>GDPR_SUBJECT_ERASED</comment> security entry.
    ...
    private function shouldProceed(SymfonyStyle $io, InputInterface $input): bool
    {
        ...
        if (
            ...
            && !$io->confirm('Irreversibly erase this identity (removes the user and its reset tokens)?', false)
        ) {
    ...
    private function eraseAndReport(SymfonyStyle $io, string $userId): int
    {
        ...
        if (!$result->erasedAnything()) {
            $io->warning('Nothing to erase — the subject has no live identity and no pending reset tokens.');

            return Command::SUCCESS;
        }

        $io->success(\sprintf(
            'Erased subject %s (identity removed: %s, reset tokens deleted: %d).',
            $result->userId,
