...
/**
 * Refuses a cross-site POST to the forgot/reset-password endpoints — the same-origin guard {@see
 * LoginOriginListener} applies to login, keyed on the two reset routes. They attend the `json` `_format`
...
final readonly class PasswordResetOriginListener
{
    private const array RESET_ROUTES = ['identity_forgot_password', 'identity_reset_password'];
    ...
    public function __invoke(RequestEvent $event): void
    {
        ...
        if (!\in_array($request->attributes->get('_route'), self::RESET_ROUTES, true)) {
            return;
        }
