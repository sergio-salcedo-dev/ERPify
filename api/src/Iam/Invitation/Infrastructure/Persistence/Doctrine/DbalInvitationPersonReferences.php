...
/**
 * {@link PersonReferenceSource} over `iam_invitation.invited_user_id` via plain DBAL — a `DISTINCT` read in
 ...
 * reported once per invitation addressed to them. `invited_user_id` is indexed, so each page is a range scan
 * over that index however large the table grows; `ORDER BY` keeps the set stable across runs so a diffing
 * alert cannot fire on noise.
 */
final readonly class DbalInvitationPersonReferences implements PersonReferenceSource
{
    ...
    {
        return $this->ids->idsOf('iam_invitation', 'invited_user_id');
    }
}
