...
interface BankAccountRepository
{
     ...
     * first left behind — a vanished row included, which resolves to `null` however the caller reached this
     * method. Subject erasure needs it because its two effects — removing the record and destroying the
     * scope's key — are one transaction whose second half cannot be replayed: a competitor that tombstones
     * the key between an unlocked read and this transaction's own flush meets the change capture sealing
     * PII under a key that no longer exists, so a request whose only honest answer is "already erased"
     * fails instead. Must run inside a transaction.
     */
