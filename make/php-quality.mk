...
# like. Syntax checking is unchanged; what stops is rejecting a construct the container understands.
#
# Repairing it was only half: a target that is a member of no aggregate is still a target nobody runs, and
# the false red was what had made that invisible. It now joins `php.quality` and `php.quality.dry-run`
# beside `php.lint.doctrine`, which shares its one constraint — both boot the kernel and so need the stack —
# so a malformed `config/*.yaml` is caught by CI rather than by whoever next compiles the container.
...
## —— Accepted-risk tag (structural half only — issue #863) ——————————————————
...
#
# Two classes, each selected by name: AcceptedRiskTagRulesGateTest falsifies the grammar/paragraph/floor
# rules against fixtures before AcceptedRiskTagGateTest trusts them on the real tree.
php.lint.accepted-risk: ## Accepted-risk tag structural gate (grammar + paragraph co-location; see #863)
	@$(PHP_TEST) bin/phpunit --filter=AcceptedRiskTagRulesGateTest
	@$(PHP_TEST) bin/phpunit --filter=AcceptedRiskTagGateTest

# PHP binds only the LAST doc comment before a declaration, so an earlier one is inert while still reading
# as documentation -- and a `@return` on it is a false type statement PHPStan reads straight past. Nothing
# else in the toolchain sees it: php-cs-fixer's `phpdoc_to_comment` is the rule for this shape, it is off
# here, and turning it on wants 48 files rewritten while catching none of the live instances (measured).
php.lint.stacked-docblock: ## Superseded-docblock gate (a doc comment PHP binds past)
	@$(PHP_TEST) bin/phpunit --filter=StackedDocblockRulesGateTest
	@$(PHP_TEST) bin/phpunit --filter=StackedDocblockGateTest

# api/src has one time source, the injected Clock port: the application layer reads it once per operation and
# hands the instant inward. A bare `new DateTimeImmutable()`, `time()`, a relative literal or Symfony's global
# clock reached statically is a second source whose instant can disagree with that reading -- and nothing else
# goes red, because a unit test injecting a clock double passes while production stamps a different instant.
php.lint.wall-clock: ## Wall-clock gate (api/src reads "now" only through the Clock port)
	@$(PHP_TEST) bin/phpunit --filter=WallClockReadRulesGateTest
	@$(PHP_TEST) bin/phpunit --filter=WallClockReadGateTest
...
.PHONY: php.stan php.stan.baseline \
        ...
        php.lint.accepted-risk php.lint.stacked-docblock php.lint.wall-clock \
